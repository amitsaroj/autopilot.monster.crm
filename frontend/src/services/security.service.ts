...
export const securityService = {
  ...
  removeIpFromWhitelist: (id: string) => api.delete(`/settings/security/ip-whitelist/${id}`),

  // Consent Tracking
  getConsentRecords: (contactId: string) => api.get(`/settings/security/consent/${contactId}`),
  recordConsent: (dto: ConsentRecordDto) => api.post('/settings/security/consent', dto),
  revokeConsent: (contactId: string, consentType: string) =>
    api.post(`/settings/security/consent/${contactId}/revoke`, { consentType }),

  // GDPR GDPR Tools
  exportGdprData: (contactId: string) => api.get(`/settings/security/gdpr/export/${contactId}`),
};
