      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16 space-y-6">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-lg max-w-2xl mx-auto leading-relaxed">
            Select a role below to explore the CRM from different perspectives. Each account is
            pre-seeded with sample data and unique permission levels.
          </p>
        </div>
        ...
        <div className="max-w-3xl mx-auto mb-12 p-6 rounded-2xl bg-white/[0.03] border border-white/[0.08] backdrop-blur-xl flex flex-col md:flex-row items-center justify-between gap-6 group hover:border-indigo-500/30 transition-all duration-300 shadow-2xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider font-semibold">
                Shared across all demo accounts
              </p>
            </div>
          </div>
          ...
          >
            <code className="text-indigo-600 dark:text-indigo-400 font-mono font-bold tracking-tight">
              SecureP@ssw0rd!
            </code>
            {copied === 'Password' ? (
              <Check className="w-4 h-4 text-green-500" />
            ) : (
              <Copy className="w-4 h-4 text-gray-400 dark:text-gray-500 group-hover/btn:text-indigo-500 dark:group-hover/btn:text-white" />
            )}
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {demoUsers.map((user, idx) => (
            ...
            >
              ...
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10">
                <div
                  className={`absolute top-0 right-0 w-32 h-32 ${user.bgIcon} rounded-full blur-[60px]`}
                />
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  <div
                    className={`w-14 h-14 rounded-2xl ${user.bgIcon} border border-white/[0.05] flex items-center justify-center shadow-inner`}
                  >
                    <user.icon className={`w-7 h-7 ${user.color}`} />
                  </div>
                  <div className="flex flex-col items-end">
                    <span className={`text-xs font-black uppercase tracking-widest ${user.color}`}>
                      {user.role}
                    </span>
          ...
          <div className="p-8 rounded-2xl border border-dashed border-white/10 flex flex-col items-center justify-center text-center space-y-4 bg-indigo-500/[0.02]">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 px-4">
              Contact our sales team for a custom tailored sandbox environment with your own
              dataset.
            </p>
            <Link
              href="/contact"
              className="text-indigo-600 dark:text-indigo-400 font-bold hover:text-indigo-500 dark:hover:text-indigo-300 transition-colors flex items-center gap-2"
            >
