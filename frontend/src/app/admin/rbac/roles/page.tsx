    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="space-y-3">
        {roles.map((role) => (
          <div
            key={role.id}
            className="rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:border-indigo-500/20 transition-all overflow-hidden"
          >
            <div
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {role.isSystem && (
                    <span className="text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded bg-white/[0.05] text-gray-500 border border-white/[0.08]">
                      SYSTEM
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-4 shrink-0">
                <div className="flex items-center gap-1.5 text-xs text-gray-400">
                  <Lock className="w-3.5 h-3.5" /> {role.permissions?.length ?? 0} perm
              ...
              <div className="px-5 pb-5 border-t border-white/[0.05] pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {(role.permissions ?? []).map((p) => (
                    <span
                      key={p.id}
                      className="px-2 py-1 rounded-lg bg-white/[0.03] border border-white/[0.05] text-[10px] font-mono text-gray-400"
                    >
                      {p.name}
                    </span>
                  ))}
