    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Role Assignment & User Access Management
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'Roles',
            value: [...new Set(users.flatMap((u) => u.roles))].length,
            icon: ShieldCheck,
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {filtered.map((user) => (
              <tr key={user.id} className="group hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-5 py-4">
                  <span
                    className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${ROLE_STYLES[user.roles[0] ?? ''] || 'bg-gray-500/10 text-gray-400 border-gray-500/20'}`}
                  >
                    {(user.roles[0] ?? 'USER').replace('_', ' ')}
                  </span>
                </td>
                <td className="px-5 py-4">
                  <span className="px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-400 border-emerald-500/20">
                    ACTIVE
                  </span>
                </td>
                <td className="px-5 py-4">
