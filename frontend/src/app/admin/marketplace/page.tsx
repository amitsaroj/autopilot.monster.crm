    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Discover & Provision New Workspace Functional Capabilities
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div
            key={cat.name}
            className={`p-8 rounded-[40px] border flex items-center justify-between group cursor-pointer hover:scale-[1.02] transition-all ${cat.bg}`}
          >
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest leading-none">
                {cat.count} Artifacts Available
              </p>
            </div>
            <cat.icon
              className={`w-10 h-10 ${cat.color} opacity-40 group-hover:opacity-100 transition-all`}
            />
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {plugins.map((plugin) => (
          <div
            key={plugin.id}
            className="p-8 rounded-[40px] bg-white/[0.02] border border-white/[0.05] hover:border-indigo-500/20 transition-all group flex flex-col justify-between relative overflow-hidden"
          >
            ...
            <div>
              <div className="flex justify-between items-start mb-8">
                ...
                {plugin.isPremium && (
                  <span className="px-3 py-1 rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/20 text-[9px] font-black uppercase tracking-widest">
                    Premium
                  </span>
                )}
              </div>
              ...
              <div className="space-y-2 mb-6">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[9px] text-gray-600 font-mono tracking-tighter uppercase whitespace-nowrap overflow-hidden text-ellipsis max-w-[200px]">
                    v{plugin.version} • {plugin.category} Artifact
                  </span>
                </div>
              </div>
              ...
              <p className="text-sm text-gray-500 leading-relaxed line-clamp-3 mb-10 font-medium">
                {plugin.description ||
                  'Provision this functional capability to enhance your workspace operational vector and communication lattice.'}
              </p>
            </div>
            ...
            <div className="space-y-6 pt-4 border-t border-white/5">
              <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                <span className="text-gray-600 flex items-center gap-1.5">
                  <Star className="w-3.5 h-3.5 text-amber-500" /> 4.9 Artifacts/5
                </span>
                <span className="text-gray-600 flex items-center gap-1.5">
                  <Users className="w-3.5 h-3.5 text-indigo-400" /> 2k Active Users
                </span>
              </div>
        ...
        {plugins.length < 5 && (
          <div className="p-10 rounded-[40px] border border-dashed border-white/10 bg-white/[0.01] flex flex-col items-center justify-center text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 max-w-[200px] mx-auto leading-relaxed mt-2 uppercase tracking-widest font-black opacity-30">
                Persistence Discovery Status: Scanning
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-500 max-w-xl font-medium">
              Extend your workspace with sandboxed functional modules. All marketplace artifacts are
              vetted for security and performance by the platform core.
            </p>
          </div>
        </div>
        <button className="px-8 py-4 bg-white/[0.05] border border-white/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-white/[0.1] shrink-0">
