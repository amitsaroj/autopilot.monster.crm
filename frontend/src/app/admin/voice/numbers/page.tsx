...
export default function AdminVoiceNumbersPage() {
  ...
  const filtered = numbers.filter(
    (n) => n.phoneNumber.includes(search) || n.country.toLowerCase().includes(search.toLowerCase()),
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'Countries',
            value: [...new Set(numbers.map((n) => n.country))].length,
            icon: Globe,
          },
      ...
      <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.05] flex items-center gap-3 focus-within:border-indigo-500/30 transition-all">
        ...
        <input
          type="text"
          placeholder="Search by number, name, or country..."
          value={search}
          ...
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex flex-col items-end gap-2">
                <span
                  className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase ${STATUS_STYLES[num.status]}`}
                >
                  {num.status}
                </span>
              </div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {Object.entries(num.capabilities)
                ...
                .map(([cap]) => (
                  <span
                    key={cap}
                    className="text-[10px] px-2 py-0.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-gray-400 font-black uppercase tracking-widest"
                  >
                    {cap}
                  </span>
                ))}
