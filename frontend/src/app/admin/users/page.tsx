...
export default function TeamManagementPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${colors[status]}`}
      >
        {status}
      </span>
    );
  ...
  const getRoleBadge = (role: string) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${colors[role] || 'bg-gray-500/10 text-gray-500 border-gray-500/20'}`}
      >
        {role.replace('_', ' ')}
      </span>
    );
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Team-Orchestrator
            </span>
          </div>
          <h1 className="text-3xl font-black text-white tracking-tight">
            Team Identity Orchestration
          </h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Manage Personnel Artifacts, RBAC Assignments & Identity persistence
          </p>
        </div>
        <button className="px-8 py-3 bg-indigo-500 hover:bg-indigo-400 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-500/20 flex items-center gap-2 group">
          <UserPlus className="w-4 h-4 group-hover:rotate-12 transition-transform" /> Provision Team
          Node
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="flex bg-white/[0.02] border border-white/5 rounded-xl p-1 shrink-0">
            <button className="p-2 rounded-lg bg-indigo-500 text-white shadow-xl">
          ...
          <button
            ...
            className="p-4 bg-white/[0.02] border border-white/5 rounded-xl text-gray-500 hover:text-white transition-all shrink-0"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {members.map((member) => (
          <div
            key={member.id}
            className="p-8 rounded-[40px] bg-white/[0.01] border border-white/[0.05] hover:border-indigo-500/20 transition-all group relative overflow-hidden flex flex-col justify-between h-[380px]"
          >
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-xl font-black text-white group-hover:text-indigo-400 transition-colors uppercase tracking-tight truncate leading-none">
                    {member.name || 'Anonymous Entity'}
                  </h3>
            ...
            <div className="space-y-6 pt-6 border-t border-white/5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-[9px] text-gray-700 font-black uppercase tracking-widest">
                  <Clock className="w-3.5 h-3.5 opacity-40 shrink-0" /> Latent{' '}
              ...
              <div className="flex gap-3">
                <button className="flex-1 py-4 rounded-2xl bg-white/[0.03] border border-white/5 text-[10px] font-black text-white uppercase tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2 group/btn">
                  <Edit2 className="w-4 h-4 text-indigo-500 group-hover/btn:scale-110 transition-transform" />{' '}
                  Edit Identity
                </button>
        ...
        <div className="p-8 rounded-[40px] border border-dashed border-white/10 bg-indigo-500/[0.01] flex flex-col items-center justify-center text-center space-y-6 group cursor-pointer hover:border-indigo-500/30 transition-all h-[380px]">
          ...
          <div>
            <h3 className="text-xl font-black text-white uppercase tracking-tighter leading-tight font-sans">
              Invite New Agent
            </h3>
            <p className="text-sm text-gray-600 max-w-[220px] mx-auto leading-relaxed mt-2 uppercase tracking-widest font-black opacity-30">
              Identity Slot: Available
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="p-10 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden">
          ...
          <div className="space-y-3 relative">
            ...
            <p className="text-sm text-gray-500 font-medium leading-relaxed max-w-sm">
              Manage how new team nodes inherit workspace artifacts. Orchestrate automated
              data-permission dispatches for high-velocity onboarding.
            </p>
          </div>
        </div>
        <div className="p-10 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden">
          ...
          <div className="space-y-3 relative">
            ...
            <p className="text-sm text-gray-500 font-medium leading-relaxed max-w-sm">
              Conduct structural identity audits to ensure absolute RBAC compliance. Track
              policy-divergences across your entire workspace identity lattice.
            </p>
          </div>
        </div>
      </div>
