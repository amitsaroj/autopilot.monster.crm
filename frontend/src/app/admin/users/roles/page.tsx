            ...
            {roles.map((role) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 relative">
                  <div
                    className={`p-3 rounded-2xl ${selectedRole?.id === role.id ? 'bg-white/20' : 'bg-white/[0.03] text-purple-400'} group-hover:scale-110 transition-transform`}
                  >
                    <Shield className="w-5 h-5" />
                  </div>
                  <div>
                    <h4
                      className={`text-sm font-black uppercase tracking-tight ${selectedRole?.id === role.id ? 'text-white' : 'text-gray-200'}`}
                    >
                      {role.name}
                    </h4>
                    <p
                      className={`text-[10px] truncate max-w-[150px] font-medium leading-none mt-1.5 ${selectedRole?.id === role.id ? 'text-white/60' : 'text-gray-600'}`}
                    >
                      {role.description}
                    </p>
                  </div>
          ...
          <div className="flex justify-between items-center mb-10 relative">
            <div>
              ...
              <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest mt-2">
                ...
                <span className="text-purple-400">
                  {selectedRole?.name || 'Inspect Role to View Permissions'}
                </span>
              </p>
            </div>
            <div className="flex gap-4">
              ...
              <button className="px-8 py-3 bg-white text-[#0b0f19] rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center gap-2 group">
                <Save className="w-4 h-4 group-hover:scale-110 transition-transform" /> Save
                Assignments
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 relative max-h-[600px] overflow-y-auto custom-scrollbar pr-4">
            {permissions.length > 0 ? (
                      ...
                      .map((p) => (
                        ...
                        >
                          <div className="space-y-1">
                            <p className="text-xs font-black text-white uppercase tracking-tight group-hover/item:text-purple-400 transition-colors">
                              {p.name}
                            </p>
                            <p className="text-[9px] text-gray-700 font-mono italic">{p.code}</p>
                          </div>
                          <div
                            className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${selectedRole?.permissions.includes(p.code) ? 'bg-purple-600 border-purple-500 text-white shadow-lg shadow-purple-900/20' : 'bg-white/[0.02] border-white/10 text-gray-800 hover:border-purple-500/40'}`}
                          >
                            {selectedRole?.permissions.includes(p.code) ? (
                              <CheckSquare className="w-5 h-5" />
                            ) : (
                              <Plus className="w-5 h-5 opacity-20 group-hover/item:opacity-40" />
                            )}
                          </div>
                        </div>
                      ))}
              ...
              <div className="col-span-2 py-32 text-center space-y-6">
                ...
                <p className="text-sm font-black text-gray-600 uppercase tracking-widest leading-none">
                  Permission Lattices Initializing...
                </p>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-10 relative">
          ...
          <div className="space-y-3">
            <h3 className="text-3xl font-black uppercase tracking-tighter leading-none">
              Structural RBAC Audit
            </h3>
            <p className="text-sm text-white/70 font-bold uppercase tracking-tight max-w-xl">
              Analyze historical RBAC dispatches and authority assignments. Inspect every structural
              policy-divergence to ensure absolute governance across the workspace node.
            </p>
          </div>
        </div>
