    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Group-Orchestrator
            </span>
          </div>
          <h1 className="text-3xl font-black text-white tracking-tight">
            Functional Team Orchestration
          </h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Manage Operational Units, Departmental persistence & Group Forensics
          </p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={fetchGroups}
            className="p-4 bg-white/[0.02] border border-white/5 rounded-xl text-gray-500 hover:text-white transition-all shrink-0 shadow-lg"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {groups.map((group) => (
          <div
            key={group.id}
            className="p-10 rounded-[60px] bg-white/[0.02] border border-white/[0.05] hover:border-indigo-500/20 transition-all group flex flex-col justify-between relative overflow-hidden h-[450px]"
          >
            ...
            <div>
              ...
              <h3 className="text-2xl font-black text-white group-hover:text-indigo-400 transition-colors uppercase tracking-tight mb-4 leading-none">
                {group.name || 'Untitled Unit'}
              </h3>
              <p className="text-sm text-gray-500 font-medium leading-relaxed line-clamp-2 uppercase tracking-wide opacity-80">
                {group.description ||
                  'No functional description provided for this operational node.'}
              </p>
              ...
              <div className="mt-8 flex items-center gap-4">
                <div className="flex -space-x-3 overflow-hidden">
                  {group.members.slice(0, 3).map((m, idx) => (
                    <div
                      key={m.id}
                      className="inline-block h-10 w-10 rounded-xl ring-4 ring-[#0b0f19] bg-white/10 flex items-center justify-center text-[10px] font-black text-indigo-400 border border-white/5 uppercase"
                    >
                      {m.name.charAt(0)}
                    </div>
                  ))}
            ...
            <div className="space-y-6 pt-8 border-t border-white/5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-[10px] text-gray-700 font-black uppercase tracking-widest">
                  <Users2 className="w-4 h-4 opacity-40 shrink-0" /> {group.members.length}{' '}
                  Identities
                </div>
                <div className="flex items-center gap-2 text-[10px] text-gray-700 font-black uppercase tracking-widest">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Groups can inherit structural RBAC policies. Enable systemic security dispatches
              across functional unit nodes.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Orchestrate team communication dispatches. Track functional unit synergy and
              collaboration pulse in real-time.
            </p>
          </div>
        </div>
      </div>
