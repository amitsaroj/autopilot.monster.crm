      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Event Triggers & Webhook Configuration
          </p>
        </div>
      ...
      <div className="space-y-3">
        {triggers.map((trigger) => {
          ...
          return (
            <div
              key={trigger.id}
              className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] hover:border-indigo-500/20 transition-all group flex items-center gap-5"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${TYPE_STYLES[trigger.type]}`}
                  >
                    {trigger.type}
                  </span>
                </div>
                <div className="flex flex-wrap items-center gap-4 text-[10px] text-gray-600">
                  <span className="flex items-center gap-1">
                    <SourceIcon className="w-3 h-3" /> {trigger.source}
                  </span>
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                <button className="p-2 rounded-lg text-gray-600 hover:text-white hover:bg-white/[0.05] transition-all">
                  <Edit2 className="w-3.5 h-3.5" />
                </button>
                <button
                  onClick={() => toast.error('System triggers cannot be deleted')}
