    ...
    const load = async () => {
      ...
      try {
        ...
        const payload = Array.isArray(res.data)
          ? res.data
          : (res.data as { data?: InvoiceRow[] })?.data;
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          {
            label: 'Successful',
            value: history.filter((h) => (h.status ?? '').toUpperCase() === 'PAID').length,
            color: 'text-blue-400',
            bg: 'bg-blue-500/10',
            icon: CheckCircle2,
          },
          {
            label: 'Failed',
            value: history.filter((h) => (h.status ?? '').toUpperCase() === 'FAILED').length,
            color: 'text-red-400',
            bg: 'bg-red-500/10',
            icon: XCircle,
          },
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {filtered.map((inv) => (
              <tr key={inv.id} className="group hover:bg-white/[0.02] transition-colors">
                <td className="px-5 py-4 text-xs font-mono font-bold text-indigo-300">
                ...
                <td className="px-5 py-4">
                  <span
                    className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${(inv.status ?? '').toUpperCase() === 'PAID' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}
                  >
                    {inv.status ?? 'UNKNOWN'}
                  </span>
                </td>
