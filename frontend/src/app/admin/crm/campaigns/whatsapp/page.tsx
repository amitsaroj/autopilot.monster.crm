...
export default function WhatsAppCampaignsPage() {
  ...
  const fetchCampaigns = async () => {
    setLoading(true);
    try {
      const res = await api.get('/crm/campaigns?type=WHATSAPP');
      const json = res.data;
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: WABA-Orchestrator
            </span>
          </div>
          <h1 className="text-3xl font-black text-white tracking-tight text-sans">
            WhatsApp Outreach Artifacts
          </h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Manage Broadcasts, Interactive Dispatches & Session Forensics
          </p>
        </div>
                ...
                <tr key={camp.id} className="group hover:bg-white/[0.02] transition-colors">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-black text-white group-hover:text-emerald-400 transition-colors uppercase tracking-tight">
                          {camp.name}
                        </p>
                    ...
                    <div className="flex flex-col items-center gap-6 text-gray-700">
                      ...
                      <div className="space-y-1">
                        <p className="text-lg font-bold text-gray-500 font-sans uppercase tracking-widest">
                          WhatsApp Lattice Dormant
                        </p>
                        <p className="text-xs text-gray-600 font-medium">
                          No WhatsApp outreach artifacts detected. Initialize your first interactive
                          dispatch to begin tracking read forensics.
                        </p>
                      </div>
                      <button
                        onClick={fetchCampaigns}
                        className="px-8 py-3 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/20"
                      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div className="p-10 rounded-[40px] bg-white/[0.02] border border-white/[0.05] space-y-6 group hover:bg-white/[0.03] transition-all relative overflow-hidden">
          ...
          <p className="text-sm text-gray-500 leading-relaxed font-medium">
            Manage high-fidelity WhatsApp Business templates with interactive buttons and media
            artifacts. Orchestrate customer activation with systemic precision.
          </p>
        </div>
        <div className="p-10 rounded-[40px] bg-white/[0.02] border border-white/[0.05] space-y-6 group hover:bg-white/[0.03] transition-all relative overflow-hidden">
          ...
          <p className="text-sm text-gray-500 leading-relaxed font-medium">
            Monitor real-time session forensics. Analyze interaction velocity and response pulse
            across your entire WhatsApp communication lattice.
          </p>
        </div>
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/60 font-black uppercase tracking-tighter italic">
                Status: E2EE Certified
              </p>
            </div>
          </div>
          <p className="text-xs text-white/80 leading-relaxed font-bold uppercase tracking-tight">
            WhatsApp artifacts are dispatched via a cryptographically secure lattice, ensuring
            absolute privacy and 99.9% delivery persistence.
          </p>
