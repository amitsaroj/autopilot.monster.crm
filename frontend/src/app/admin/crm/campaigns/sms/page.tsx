...
import { 
  ...
  Phone, Send, MessageSquare, Smartphone,
  BarChart3, Clock, Edit2, MoreVertical,
  Target, Rocket, PieChart, 
  ZapOff, Lock, User, Hash
} from 'lucide-react';
...
export default function SMSCampaignsPage() {
  ...
  const fetchCampaigns = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/v1/crm/campaigns?type=SMS');
      const json = await res.json();
      if (json.data) setCampaigns(json.data);
    } catch (e) {
      toast.error('Failed to synchronize SMS outreach artifacts');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
           <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">Node: Telecom-Orchestrator</span>
           </div>
           <h1 className="text-3xl font-black text-white tracking-tight text-sans">SMS Outreach Artifacts</h1>
           <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">Manage Global SMS Broadcasts, Alphanumeric Dispatches & Delivery Forensics</p>
        </div>
        <div className="flex items-center gap-3">
           <button className="px-8 py-3 bg-amber-500 hover:bg-amber-400 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-xl shadow-amber-500/20 flex items-center gap-2 group">
              <Plus className="w-4 h-4 group-hover:rotate-90 transition-all opacity-60" /> Provision SMS Dispatch
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         {[
           { label: 'Dispatch Velocity', value: '142K', icon: Send, color: 'text-indigo-500', bg: 'bg-indigo-500/10' },
           { label: 'Delivery Forensics', value: '98.8%', icon: ShieldCheck, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
           { label: 'Mean CPC', value: '$0.012', icon: BarChart3, color: 'text-amber-500', bg: 'bg-amber-500/10' },
           { label: 'Blocked Artifacts', value: '0.2%', icon: ZapOff, color: 'text-red-500', bg: 'bg-red-500/10' },
         ].map((stat) => (
                    ...
                    <tr key={camp.id} className="group hover:bg-white/[0.02] transition-colors">
                       <td className="px-8 py-6">
                          <div className="flex items-center gap-4">
                             <div className="w-12 h-12 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center text-amber-500 group-hover:bg-amber-500 group-hover:text-white transition-all">
                                <Phone className="w-6 h-6" />
                             </div>
                             <div>
                                <p className="text-sm font-black text-white group-hover:text-amber-400 transition-colors uppercase tracking-tight">{camp.name}</p>
                       ...
                       <td className="px-8 py-6">
                          <div className="flex items-center gap-6">
                             ...
                             <div>
                                <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-1">Failed</p>
                                <p className="text-xs font-black text-red-400">{camp.stats?.failed || 0}%</p>
                             </div>
                          </div>
                       </td>
                       <td className="px-8 py-6">
                       ...
                       <td className="px-8 py-6 text-right">
                          <div className="flex items-center justify-end gap-2 text-gray-500">
                             ...
                             <button className="p-2.5 rounded-xl bg-white/[0.03] border border-white/10 hover:text-amber-400 transition-all">
                    ...
                    <tr>
                      <td colSpan={5} className="px-8 py-24 text-center">
                         <div className="flex flex-col items-center gap-6 text-gray-700">
                            ...
                            <div className="space-y-1">
                               <p className="text-lg font-bold text-gray-500 font-sans uppercase tracking-widest">SMS Lattice Dormant</p>
                               <p className="text-xs text-gray-600 font-medium">No SMS outreach artifacts detected. Initialize your first alphanumeric dispatch to begin tracking delivery forensics.</p>
                            </div>
                            <button onClick={fetchCampaigns} className="px-8 py-3 bg-amber-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-400 transition-all shadow-xl shadow-amber-500/20">
                               Initialize SMS Dispatch
                            </button>
                         </div>
                      </td>
                    </tr>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
         <div className="p-10 rounded-[40px] bg-white/[0.02] border border-white/[0.05] space-y-6 group hover:bg-white/[0.03] transition-all relative overflow-hidden">
            ...
            <div className="flex items-center gap-4">
               <div className="p-4 rounded-3xl bg-amber-500/10 text-amber-500 border border-amber-500/20 group-hover:bg-amber-500 group-hover:text-white transition-all text-center">
                  <Hash className="w-8 h-8" />
               </div>
               <h3 className="text-xl font-black text-white uppercase tracking-tighter text-sans leading-none">Sender ID Registry</h3>
            </div>
            <p className="text-sm text-gray-500 leading-relaxed font-medium">
               Manage global sender identifiers and alphanumeric persistence artifacts. Orchestrate regional regulatory compliance with systemic precision.
            </p>
         </div>
         <div className="p-10 rounded-[40px] bg-white/[0.02] border border-white/[0.05] space-y-6 group hover:bg-white/[0.03] transition-all relative overflow-hidden">
            ...
            <div className="flex items-center gap-4">
               ...
               <h3 className="text-xl font-black text-white uppercase tracking-tighter text-sans leading-none">Global Coverage</h3>
               <span className="px-2 py-0.5 border border-emerald-500/20 text-[8px] font-black text-emerald-500 uppercase rounded-md">200+ Nodes</span>
            </div>
            <p className="text-sm text-gray-500 leading-relaxed font-medium">
               Monitor local telecom delivery pulse across 200+ global nodes. Analyze transit latency and route persistence artifacts in real-time.
            </p>
         </div>
         <div className="p-10 rounded-[40px] bg-amber-500 to-orange-600 text-white space-y-4 shadow-2xl shadow-amber-500/20 relative overflow-hidden group">
            ...
            <div className="flex items-center gap-4 mb-2">
               ...
               <div>
                  <h4 className="font-black text-sm uppercase tracking-widest leading-tight">Telecom Lattice</h4>
                  <p className="text-[10px] text-white/60 font-black uppercase tracking-tighter italic">Status: Carrier Certified</p>
               </div>
            </div>
            <p className="text-xs text-white/80 leading-relaxed font-bold uppercase tracking-tight">
               SMS artifacts are dispatched via a carrier-grade telecom lattice, ensuring 99.9% delivery persistence and real-time failure forensics.
            </p>
