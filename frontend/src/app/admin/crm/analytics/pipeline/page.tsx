    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
      ...
      <div className="space-y-12 py-10 px-8 rounded-[60px] bg-white/[0.01] border border-white/[0.05] relative overflow-hidden group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 relative">
          {metrics.length === 0 ? (
            <p className="col-span-full text-sm text-gray-500 text-center py-16">
            ...
            metrics.map((step, idx) => {
              ...
              const conversionRate =
                prevCount > 0 ? Math.round((step.value / prevCount) * 100) : 100;
              return (
                <div
                  key={step.name}
                  className="flex flex-col items-center group/card transition-all cursor-pointer"
                >
                  <div className="w-full p-8 rounded-[40px] bg-white/[0.02] border border-white/[0.05] group-hover/card:bg-indigo-500/10 group-hover/card:border-indigo-500/40 transition-all text-center space-y-4 relative overflow-hidden h-[240px] flex flex-col justify-center">
                    <div className="flex flex-col items-center gap-2">
                      <span className="text-[10px] text-gray-600 font-black uppercase tracking-widest leading-none group-hover/card:text-indigo-400">
                        Stage {idx + 1}
                      </span>
                      <h4 className="text-sm font-black text-white uppercase tracking-tighter line-clamp-2">
                        {step.name}
                      </h4>
                    </div>
                    <div className="space-y-1">
                      <p className="text-2xl font-black text-white uppercase tracking-tighter">
                        ${(step.amount / 1000).toFixed(0)}k
                      </p>
                      <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">
                        {step.value} Opportunity Nodes
                      </p>
                    </div>
                  </div>
          ...
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest mt-2">
                Persistence Metric: Mean Resolution Time
              </p>
            </div>
          ...
          <div className="space-y-8">
            {metrics.length === 0 ? (
              ...
              metrics.map((step, idx) => (
                <div
                  key={step.name}
                  className="flex items-center justify-between p-6 rounded-3xl bg-white/[0.01] border border-white/5 group/row hover:bg-white/[0.03] transition-all"
                >
                  ...
                  <div className="text-right">
                    <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-0.5">
                      Deal Count
                    </p>
                    <p className="text-sm font-black text-white uppercase tracking-tighter">
                      {step.value}
          ...
          <div className="p-6 rounded-[32px] bg-white/[0.03] border border-white/10 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <h4 className="text-[11px] font-black text-white uppercase tracking-widest">
                Optimized Outcome Prompt
              </h4>
            </div>
            <p className="text-[10px] text-gray-600 leading-relaxed font-medium">
              "Increase Proposal Dispatch interaction pulse to reduce 14% artifact abandonment rate
              in the fiscal lattice."
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div className="p-10 rounded-[60px] bg-white/[0.02] border border-white/[0.05] space-y-6 group hover:bg-white/[0.03] transition-all relative overflow-hidden">
          ...
          <div className="flex items-center gap-6">
            ...
            <h3 className="text-xl font-black text-white uppercase tracking-tighter text-sans leading-none">
              Fiscal Flow analytics
            </h3>
          </div>
          <p className="text-sm text-gray-500 leading-relaxed font-medium">
            Monitor how fiscal value artifacts propagate through the workspace pipeline. Orchestrate
            stage-level revenue forecasting with absolute precision.
          </p>
        </div>
        <div className="p-10 rounded-[60px] bg-white/[0.02] border border-white/[0.05] space-y-6 group hover:bg-white/[0.03] transition-all relative overflow-hidden">
          ...
          <p className="text-sm text-gray-500 leading-relaxed font-medium">
            Analyze the speed of opportunity transition artifacts. Identify structural friction
            points in your conversion funnel and optimize dispatch patterns.
          </p>
        </div>
          ...
          <div className="flex items-center gap-6 relative">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/60 font-black uppercase tracking-widest mt-2 px-1">
                Status: Velocity Optimized
              </p>
            </div>
          </div>
          <p className="text-xs text-white/90 leading-relaxed font-bold uppercase tracking-tight relative">
            Your workspace funnel transition velocity is currently performing in the top 3% of the
            platform's global cluster benchmarks.
          </p>
          <div className="flex gap-4 relative">
            <button
              onClick={fetchPipelineAnalytics}
              className="flex-1 py-5 bg-white text-[#0b0f19] rounded-3xl text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-2xl"
            >
              Re-Analyze
            </button>
