  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setThreads(
          items.map((c: WhatsAppConversation) => ({
            id: c.phone,
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Unified Omnichannel Conversation Hub
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          {
            label: 'Pending',
            value: threads.filter((t) => t.status === 'PENDING').length,
            color: 'text-amber-400',
            bg: 'bg-amber-500/10',
          },
          {
            label: 'Resolved Today',
            value: threads.filter((t) => t.status === 'RESOLVED').length,
      ...
      <div className="space-y-2">
        {filtered.map((thread) => {
          ...
          return (
            <div
              key={thread.id}
              className={`p-5 rounded-2xl border transition-all group cursor-pointer hover:bg-white/[0.04] ${thread.unread > 0 ? 'bg-white/[0.03] border-indigo-500/20' : 'bg-white/[0.01] border-white/[0.04]'}`}
            >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <p
                        className={`text-sm font-bold ${thread.unread > 0 ? 'text-white' : 'text-gray-300'}`}
                      >
                        {thread.contact}
                      </p>
                      <div
                        className={`flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] font-black ${channelColor}`}
                      >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 ml-3 shrink-0">
                      <span
                        className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${STATUS_STYLES[thread.status]}`}
                      >
                        {thread.status}
                      </span>
