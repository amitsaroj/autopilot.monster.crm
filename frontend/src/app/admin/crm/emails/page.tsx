    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          {
            label: 'Received',
            value: emails.filter((e) => e.direction === 'INBOUND').length,
            color: 'text-emerald-400',
            bg: 'bg-emerald-500/10',
          },
          {
            label: 'Sent',
            value: emails.filter((e) => e.direction === 'OUTBOUND').length,
            color: 'text-blue-400',
            bg: 'bg-blue-500/10',
          },
          {
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${s.bg.replace('bg-', 'bg-').replace('/10', '')}`}
            />
            <div>
      ...
      <div className="flex gap-3">
        ...
        >
          {['All', 'RECEIVED', 'SENT', 'DRAFT', 'BOUNCED'].map((s) => (
      ...
      <div className="space-y-2">
        {filtered.map((email) => (
          <div
            key={email.id}
            className={`p-5 rounded-2xl border transition-all group cursor-pointer hover:bg-white/[0.04] ${email.isRead ? 'bg-white/[0.01] border-white/[0.04]' : 'bg-white/[0.03] border-indigo-500/20'}`}
          >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-3 mb-1">
                  <div className="flex items-center gap-3">
                    <p
                      className={`text-sm font-bold ${email.isRead ? 'text-gray-300' : 'text-white'}`}
                    >
                      {email.direction === 'INBOUND' ? email.from : email.to}
                    </p>
                    <span
                      className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${STATUS_STYLES[email.direction === 'INBOUND' ? 'RECEIVED' : 'SENT']}`}
                    >
                      {email.direction}
                    </span>
                  </div>
                ...
                <p
                  className={`text-sm mb-1 ${email.isRead ? 'text-gray-500' : 'text-gray-300 font-medium'}`}
                >
                  {email.subject}
                </p>
