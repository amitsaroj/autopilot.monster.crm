  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Page Total', value: products.length, icon: Package },
          {
            label: 'Active (page)',
          ...
          { label: 'Page Value', value: `$${totalRevenue.toLocaleString()}`, icon: DollarSign },
        ].map((s) => (
          <div
            key={s.label}
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {products.map((product) => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase ${STATUS_STYLES[product.status] ?? STATUS_STYLES.INACTIVE}`}
                  >
                    {product.status}
                  </span>
                </div>
            ...
            {products.length === 0 && (
              <div className="col-span-full py-20 text-center text-gray-500 font-black text-xs uppercase tracking-widest">
                No products found.
              </div>
            )}
          </div>
