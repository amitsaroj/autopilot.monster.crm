...
export default function WorkspaceCommunicationsDashboard() {
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      id: '1',
      title: 'Identity Lattice Synchronized',
      message:
        'Workspace identity artifacts have been committed to the global persistence lattice.',
      type: 'SUCCESS',
      read: false,
    ...
    {
      id: '2',
      title: 'Monetization Threshold Alert',
      message: 'Subscription billing cycle failed to initialize for cluster node: Alpha-42.',
      type: 'ERROR',
      read: false,
    ...
    {
      id: '3',
      title: 'Structural Upgrade Pending',
      message: 'New platform engine artifacts (v4.2.0) are available for deployment.',
      type: 'INFO',
      read: true,
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Notification-Orchestrator
            </span>
          </div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Orchestrate Structural alerts, systemic Events & engagement forensics
          </p>
        </div>
      ...
      <div className="space-y-6">
        {notifications.map((notif) => (
          <div
            key={notif.id}
            className={`p-10 rounded-[60px] bg-white/[0.01] border transition-all group flex flex-col lg:flex-row items-center gap-10 relative overflow-hidden ${!notif.read ? 'border-indigo-500/20 bg-indigo-500/[0.01]' : 'border-white/[0.05]'}`}
          >
            ...
            <div
              className={`w-20 h-20 rounded-[32px] border flex items-center justify-center transition-all shrink-0 shadow-2xl ${getStatusColor(notif.type)} group-hover:scale-105`}
            >
              {notif.type === 'ERROR' ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-4 mb-3 flex-wrap">
                ...
                <span className="text-[9px] text-gray-600 font-black uppercase tracking-widest bg-white/[0.03] border border-white/10 px-2 py-0.5 rounded-md shrink-0">
                  {notif.source} dispatch
                </span>
              </div>
              <p className="text-sm text-gray-400 font-medium leading-relaxed max-w-2xl uppercase tracking-tight opacity-80">
                {notif.message}
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mt-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative text-sans">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80 pt-2 font-sans">
              Execute structural workspace announcements. Maintain absolute interaction persistence
              across all functional unit members.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative text-sans">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80 pt-2 font-sans">
              Analyze systemic engagement forensics. Identify structural communication divergences
              and initialize recovery dispatches in real-time.
            </p>
          </div>
        </div>
      </div>
