...
export default function SupportTicketsPage() {
  ...
  const getPriorityBadge = (priority: string) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${colors[priority]}`}
      >
        {priority} PRIORITY
      </span>
    );
                  ...
                  <td className="px-8 py-6">
                    <div className="space-y-1">
                      ...
                      <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest">
                        Latent {new Date(t.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                  </td>
                  <td className="px-8 py-6">{getPriorityBadge(t.priority)}</td>
                    ...
                    <div className="flex flex-col items-center gap-6 text-gray-700">
                      ...
                      <div className="space-y-1">
                        <p className="text-lg font-bold text-gray-500 font-sans uppercase tracking-widest">
                          Support Lattice Calm
                        </p>
                        <p className="text-xs text-gray-600 font-medium">
                          No unresolved support artifacts detected in this workspace cluster.
                          Customer satisfaction pulse is currently optimal.
                        </p>
                      </div>
                      <button
                        onClick={fetchTickets}
                        className="px-8 py-3 bg-white text-[#0b0f19] rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-white/10"
                      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="p-10 rounded-[60px] bg-white/[0.02] border border-white/[0.05] space-y-8 group relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="space-y-6">
            ...
            <p className="text-sm text-gray-500 leading-relaxed font-medium">
              Utilize workspace self-service artifacts to reduce ticket volume. Orchestrate
              automated resolution patterns using platform-wide intelligence dispatches.
            </p>
          </div>
          <div className="pt-8 border-t border-white/5 flex items-center justify-between">
            <span className="text-[10px] text-gray-600 font-black uppercase tracking-widest">
              Total KB Artifacts: 42
            </span>
        ...
        <div className="p-10 rounded-[60px] bg-indigo-500 to-purple-600 text-white space-y-8 shadow-2xl shadow-indigo-500/20 relative overflow-hidden group">
          ...
          <p className="text-sm text-white/80 leading-relaxed font-bold uppercase tracking-tight">
            Enable automated ticket resolution dispatches. AI agents can autonomously resolve
            level-1 artifacts using your workspace's deep knowledge lattice.
          </p>
