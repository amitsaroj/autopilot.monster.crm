...
import { Save, Loader2, ShieldCheck, HardDrive, Activity, Cloud, Download } from 'lucide-react';
      ...
      <div className="rounded-[40px] border border-white/[0.05] bg-white/[0.01] overflow-hidden">
        <div className="px-8 py-6 border-b border-white/[0.05] flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => void handleExport('contacts')}
              className="px-4 py-2 rounded-xl bg-white/5 text-xs font-black uppercase tracking-widest text-gray-300 hover:bg-white/10 flex items-center gap-2"
            >
              <Download className="w-3 h-3" /> Export Contacts
            </button>
            <button
              type="button"
              onClick={() => void handleExport('deals')}
              className="px-4 py-2 rounded-xl bg-white/5 text-xs font-black uppercase tracking-widest text-gray-300 hover:bg-white/10 flex items-center gap-2"
            >
        ...
        <div className="divide-y divide-white/[0.03]">
          {backups.map((backup) => (
            <div
              key={backup.id}
              className="px-8 py-5 flex items-center justify-between hover:bg-white/[0.02]"
            >
              <div>
                <p className="text-sm font-bold text-white">
                  {backup.name ?? backup.type ?? backup.id}
                </p>
