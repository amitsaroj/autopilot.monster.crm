  ...
  const providers = [
    {
      ...
      desc: 'Manage payments and subscription webhooks.',
    },
    {
      name: 'Twilio',
      icon: MessageSquare,
      color: 'text-red-500',
      desc: 'Orchestrate SMS and Voice communication.',
    },
    {
      name: 'OpenAI',
      icon: Bot,
      color: 'text-emerald-500',
      desc: 'Power AI assistants and CRM intelligence.',
    },
    {
      name: 'VAPI',
      icon: Zap,
      color: 'text-amber-500',
      desc: 'Real-time AI voice conversation artifacts.',
    },
  ];
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Manage External Artifacts & Service Connections
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {providers.map((p) => (
          <div
            key={p.name}
            className="p-6 rounded-[32px] bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] hover:border-indigo-500/20 transition-all group cursor-pointer relative overflow-hidden"
          >
            ...
            <div
              className={`w-12 h-12 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center mb-4 ${p.color} group-hover:bg-indigo-500 group-hover:text-white transition-all`}
            >
            ...
            <p className="text-[10px] text-gray-600 leading-relaxed font-medium uppercase tracking-tighter">
              {p.desc}
            </p>
          </div>
        ))}
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <p className="text-[10px] text-gray-600 font-mono uppercase tracking-tighter">
                        Last synced: {new Date(s.createdAt).toLocaleDateString()}
                      </p>
                ...
                <div className="p-20 text-center space-y-6">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-bold text-gray-500">
                      No integration artifacts provisioned
                    </p>
                    <p className="text-[10px] text-gray-600 uppercase tracking-widest px-1">
                      Connect a provider to initialize the workspace Connectivity Lattice.
                    </p>
                  </div>
                </div>
              )}
        ...
        <div className="space-y-8">
          ...
          <div className="p-8 rounded-[40px] bg-indigo-500/10 border border-indigo-500/20 space-y-6 relative overflow-hidden group">
            ...
            <p className="text-xs text-gray-400 leading-relaxed font-medium">
              Workspace API artifacts are cryptographically locked and only decrypted during
              systemic execution. Platform admins cannot view your unmasked secrets.
            </p>
          </div>
          ...
          <div className="p-8 rounded-[40px] bg-white/[0.02] border border-white/[0.05] shadow-2xl space-y-4">
            ...
            <p className="text-[9px] text-gray-600 font-black uppercase tracking-tighter">
              94% Integration Artifact Success Rate
            </p>
          </div>
        </div>
      </div>
