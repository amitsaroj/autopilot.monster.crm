...
const backups = [
  {
    id: 1,
    name: 'Auto Backup — Oct 15, 2024',
    type: 'Automatic',
    size: '18.4 MB',
    status: 'Complete',
    created: 'Oct 15, 2024 02:00 AM',
  },
  {
    id: 2,
    name: 'Manual Backup — Oct 10',
    type: 'Manual',
    size: '17.8 MB',
    status: 'Complete',
    created: 'Oct 10, 2024 10:32 AM',
  },
  {
    id: 3,
    name: 'Auto Backup — Oct 8, 2024',
    type: 'Automatic',
    size: '17.2 MB',
    status: 'Complete',
    created: 'Oct 8, 2024 02:00 AM',
  },
];
    ...
    <div className="space-y-6 animate-fade-in max-w-3xl">
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        <h2 className="text-sm font-semibold flex items-center gap-2">
          <RefreshCw className="h-4 w-4 text-[hsl(246,80%,60%)]" />
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select className="w-full px-3 py-2 text-sm border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-[hsl(246,80%,60%)]">
              {['Daily at 2:00 AM', 'Twice daily', 'Weekly (Sunday 2am)', 'Monthly'].map((o) => (
          ...
          <div>
            ...
            <select className="w-full px-3 py-2 text-sm border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-[hsl(246,80%,60%)]">
              {['7 days', '14 days', '30 days', '90 days'].map((o) => (
                <option key={o}>{o}</option>
              ))}
            </select>
          </div>
        </div>
        <button className="flex items-center gap-2 px-4 py-2 border border-border rounded-lg hover:bg-muted text-sm transition-colors">
