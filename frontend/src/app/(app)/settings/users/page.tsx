    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-sm font-bold text-gray-500 uppercase tracking-widest mt-1">
            Manage access privileges and user nodes
          </p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="p-6 rounded-3xl bg-[#0b0f19] border border-white/5 flex flex-col justify-center relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-white">
            {
              users.filter((u) => u.roles.includes('ADMIN') || u.roles.includes('TENANT_ADMIN'))
                .length
            }
          </p>
        </div>
                  ...
                  <td className="p-6">
                    <div className="flex gap-2">
                      {member.roles.map((role) => (
                        <span
                          key={role}
                          className="px-2 py-1 rounded-md bg-white/[0.03] border border-white/10 text-[9px] font-black text-gray-400 uppercase tracking-widest"
                        >
                          {role.replace('_', ' ')}
                        </span>
                      ))}
          ...
          {users.length === 0 && (
            <div className="p-12 text-center">
              <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">
                No subordinate identities provisioned
              </p>
            </div>
          )}
        </div>
