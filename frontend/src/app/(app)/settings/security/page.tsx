...
export default function SecuritySettingsPage() {
      ...
      setWhitelist([
        {
          ...
          description: 'HQ Office Wifi',
          createdAt: new Date().toISOString(),
        },
        {
          id: 'ip_2',
          ipAddress: '10.0.0.42',
          description: 'Dev Local VPN Server',
          createdAt: new Date().toISOString(),
        },
      ]);
    } finally {
  ...
  const handleGdprErase = async () => {
    ...
    if (
      !confirm(
        'WARNING: Erasing this contact is permanent and complies with GDPR Right to be Forgotten. Do you want to proceed?',
      )
    )
      return;
    try {
    ...
    <div className="space-y-6 animate-fade-in max-w-7xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Configure system security boundaries, track user data consent logs, and process GDPR
          compliance requests.
        </p>
      </div>
                  ...
                  <div className="space-y-4">
                    {consentLogs.length === 0 ? (
                      ...
                      consentLogs.map((log) => (
                        <div
                          key={log.id}
                          className="p-4 border border-border rounded-xl bg-muted/5 space-y-2"
                        >
                ...
                <form onSubmit={handleRecordConsent} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConsentForm({ ...consentForm, contactId: e.target.value })
                      }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setConsentForm({ ...consentForm, consentType: e.target.value })
                      }
                      className="w-full bg-background border border-input rounded-lg px-3 py-1.5 text-sm mt-1 focus:outline-none focus:ring-2 focus:ring-primary"
                    >
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <input
                      ...
                      onChange={(e) =>
                        setConsentForm({ ...consentForm, granted: e.target.checked })
                      }
            ...
            <div className="space-y-6 max-w-4xl">
              ...
              <div className="bg-card border border-border rounded-xl p-6 shadow-sm space-y-6">
                <div className="flex items-center gap-2 border-b border-border pb-3">
                  ...
                  <h2 className="font-bold text-foreground text-sm">
                    GDPR Right to Object & Erasure Tools
                  </h2>
                </div>
                ...
                <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-600/90 dark:text-red-400/90 mt-0.5">
                      Executing an erase command anonymizes or removes the contact records entirely
                      from the relational database in compliance with GDPR Art 17 (Right to be
                      Forgotten).
                    </p>
                  </div>
                </div>
                  ...
                  <div className="border border-border rounded-xl p-4 bg-muted/10 space-y-4">
                    <div className="flex items-center justify-between border-b border-border/50 pb-2">
                      <h4 className="text-xs font-bold text-foreground uppercase tracking-wider">
                        Contact GDPR Data Package
                      </h4>
                      <span className="text-[10px] text-muted-foreground">
                        Exported at: {new Date(exportedData.exportedAt).toLocaleTimeString()}
                      </span>
                    </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div>
                          ...
                          <span className="font-medium text-foreground">
                            {exportedData.personalDetails?.firstName}{' '}
                            {exportedData.personalDetails?.lastName}
                          </span>
                        </div>
                        <div>
                          Email address:{' '}
                          <span className="font-medium text-foreground font-mono">
                            {exportedData.personalDetails?.email}
                          </span>
                        </div>
                        <div>
                          Phone contact:{' '}
                          <span className="font-medium text-foreground font-mono">
                            {exportedData.personalDetails?.phone}
                          </span>
                        </div>
                        <div>
                          Company name:{' '}
                          <span className="font-medium text-foreground">
                            {exportedData.personalDetails?.company}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {exportedData.consents?.map((c: any, idx: number) => (
                          ...
                          >
                            ...
                            <span
                              className={`font-bold ${c.granted ? 'text-green-500' : 'text-red-500'}`}
                            >
