      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Configure acquisition stages and conversion logic flows
          </p>
        </div>
        <button
          onClick={openCreate}
                ...
                <div className="space-y-3 pt-4 border-t border-white/5">
                  {pipeline.stages
                    ...
                    .map((stage, idx) => (
                      <div
                        key={stage.id}
                        className="flex items-center gap-3 text-[10px] text-gray-400 font-black uppercase tracking-widest"
                      >
                        <span className="w-5 h-5 rounded-md bg-white/5 flex items-center justify-center text-[8px] text-gray-500 border border-white/5">
                          {idx + 1}
                        </span>
                        {stage.name}
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-xl p-8 rounded-[40px] bg-[#0b0f19] border border-white/10 shadow-2xl animate-in zoom-in-95">
            <h2 className="text-2xl font-black text-white uppercase tracking-tighter mb-8 flex items-center gap-3">
              ...
              {editingId ? 'Reconfigure Flow' : 'Initialize Flow'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center gap-3 p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                <input
                  ...
                  className="w-4 h-4 rounded bg-white/5 border-white/10 text-indigo-500 focus:ring-0 cursor-pointer"
                />
                <label
                  htmlFor="isDefault"
