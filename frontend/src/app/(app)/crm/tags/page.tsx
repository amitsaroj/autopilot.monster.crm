      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Manage entity classification nodes and visual markers
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {tags.map((tag) => (
            <div
              key={tag.id}
              className="p-6 rounded-[32px] bg-white/[0.02] border border-white/[0.05] hover:border-indigo-500/20 transition-all group relative overflow-hidden backdrop-blur-sm"
            >
              ...
              <div className="flex items-center gap-2">
                <span className="text-[9px] font-bold text-gray-500 uppercase tracking-tighter">
                  Identity: {tag.id.slice(0, 8)}
                </span>
                <span className="text-[9px] font-black px-1.5 py-0.5 rounded-md bg-white/5 text-gray-400 uppercase tracking-tighter">
                  {tag.type}
                </span>
              </div>
              <div
          ...
          <div className="w-full max-w-lg p-8 rounded-[40px] bg-[#0b0f19] border border-white/10 shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl pl-12 pr-5 py-4 text-sm text-white outline-none focus:border-indigo-500/40 uppercase tracking-widest"
                ...
                <div className="flex flex-wrap gap-3">
                  {colors.map((c) => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl transition-all border-2 ${formData.color === c ? 'border-white scale-110 shadow-lg' : 'border-transparent opacity-50 hover:opacity-100'}`}
              ...
              <div className="space-y-2">
                ...
                >
                  <option value="GENERIC" className="bg-[#0b0f19]">
                    Omni-Dimensional (Generic)
                  </option>
