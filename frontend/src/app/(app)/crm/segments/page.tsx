      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Manage dynamic audience clusters and logical entity groupings
          </p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="px-8 py-3 bg-violet-600 hover:bg-violet-500 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-xl shadow-violet-500/20 flex items-center gap-2 group"
        >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {segments.map((segment) => (
            <div
              key={segment.id}
              className="p-8 rounded-[40px] bg-white/[0.02] border border-white/[0.05] hover:border-violet-500/20 transition-all group flex flex-col justify-between relative overflow-hidden backdrop-blur-sm"
            >
              ...
              <div>
                ...
                <h3 className="text-xl font-black text-white group-hover:text-violet-400 transition-colors uppercase tracking-tight mb-2 leading-none uppercase tracking-tight mb-2">
                  {segment.name}
                </h3>
                ...
                <div className="space-y-4 pt-4 border-t border-white/5">
                  <div className="flex items-center gap-4 text-[10px] text-gray-500 font-black uppercase tracking-widest">
                    <Zap className="w-4 h-4 text-amber-500 opacity-40 shrink-0" />{' '}
                    {segment.description || 'No descriptive logic assigned.'}
                  </div>
                  <div className="flex items-center gap-4 text-[10px] text-gray-400 font-black uppercase tracking-widest bg-white/[0.02] p-3 rounded-xl border border-white/5">
                    ...
                    {segment.rules?.conditions?.length || 0} Rule Conditions
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="w-full max-w-xl p-8 rounded-[40px] bg-[#0b0f19] border border-white/10 shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-violet-500/40"
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-violet-500/40 appearance-none cursor-pointer"
                  >
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">
                    Logical Scope Description
                  </label>
                  <textarea
                    value={formData.description}
                    onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-violet-500/40 min-h-[100px] resize-none"
                    placeholder="Targeting contacts with high revenue potential..."
                  />
                </div>
              </div>
              <div className="pt-6 border-t border-white/5 flex gap-4">
                ...
                <button
                  type="submit"
                  className="flex-1 py-4 bg-violet-600 hover:bg-violet-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-violet-500/20"
                >
                  Execute Clustering
                </button>
              </div>
