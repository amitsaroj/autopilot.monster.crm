      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Manage persistent knowledge artifacts and contextual intelligence
          </p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="px-8 py-3 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-xl shadow-amber-500/20 flex items-center gap-2 group"
        >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredNotes.map((note) => (
            <div
              key={note.id}
              className="p-8 rounded-[40px] bg-white/[0.02] border border-white/[0.05] hover:border-amber-500/20 transition-all group flex flex-col justify-between relative overflow-hidden backdrop-blur-sm"
            >
              ...
              <div>
                <div className="flex justify-between items-start mb-6">
                  <div className="w-14 h-14 rounded-[20px] bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 group-hover:bg-amber-500 group-hover:text-white group-hover:border-amber-400 transition-all shadow-2xl relative">
                    ...
                    {note.isPinned && (
                      <Bookmark className="absolute -top-1 -right-1 w-4 h-4 text-amber-400 fill-amber-400" />
                    )}
                  </div>
                  <button
                ...
                <h3 className="text-xl font-black text-white group-hover:text-amber-400 transition-colors uppercase tracking-tight mb-4 leading-none">
                  {note.title || 'Incipient Artifact'}
                </h3>
          ...
          {filteredNotes.length === 0 && (
            <div className="col-span-full py-20 text-center">
              <p className="text-gray-500 font-black text-xs uppercase tracking-widest">
                No knowledge artifacts detected in the fabric.
              </p>
            </div>
          )}
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-amber-500/40"
                  placeholder="e.g. Strategic Follow-up Context"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-amber-500/40 min-h-[200px] resize-none"
                  placeholder="Enter persistent intelligence..."
                />
              </div>
              ...
              <div className="pt-6 border-t border-white/5 flex gap-4">
                ...
                <button
                  type="submit"
                  className="flex-1 py-4 bg-amber-600 hover:bg-amber-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-amber-500/20"
                >
