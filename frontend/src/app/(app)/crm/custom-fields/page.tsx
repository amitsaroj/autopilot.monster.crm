      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Manage dynamic schema extensions and entity properties
          </p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-xl shadow-blue-500/20 flex items-center gap-2 group"
        >
          ...
          {fields.map((field) => (
            ...
            >
              ...
              <div>
                ...
                <h3 className="text-xl font-black text-white group-hover:text-blue-400 transition-colors uppercase tracking-tight mb-2 leading-none uppercase tracking-tight mb-2">
                  {field.label}
                </h3>
                ...
                <div className="space-y-3 pt-4 border-t border-white/5">
                  ...
                  <div className="flex items-center gap-3 text-[10px] text-gray-400 font-black uppercase tracking-widest">
                    <div className="opacity-40 shrink-0">{getTypeIcon(field.type)}</div> Type:{' '}
                    {field.type}
                  </div>
                </div>
              </div>
          ...
          {fields.length === 0 && (
            <div className="col-span-full py-20 text-center">
              <p className="text-gray-500 font-black text-xs uppercase tracking-widest">
                No custom schema nodes detected in the lattice.
              </p>
            </div>
          )}
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-1 space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-blue-500/40"
                ...
                <div className="col-span-1 space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-blue-500/40 font-mono text-[10px]"
                ...
                <div className="col-span-1 space-y-2">
                  ...
                  <select
                    ...
                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-blue-500/40 appearance-none cursor-pointer"
                  >
                    <option value="TEXT" className="bg-[#0b0f19]">
                      TEXT (String)
                    </option>
                    <option value="NUMBER" className="bg-[#0b0f19]">
                      NUMBER (Integer/Float)
                    </option>
                    <option value="DATE" className="bg-[#0b0f19]">
                      DATE (ISO-8601)
                    </option>
                    <option value="BOOLEAN" className="bg-[#0b0f19]">
                      BOOLEAN (True/False)
                    </option>
                  </select>
                </div>
                <div className="col-span-1 space-y-2">
                  ...
                  <select
                    ...
                    className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-blue-500/40 appearance-none cursor-pointer"
                  >
                    <option value="LEAD" className="bg-[#0b0f19]">
                      Acquisition Vector (Lead)
                    </option>
                    <option value="CONTACT" className="bg-[#0b0f19]">
                      Human Entity (Contact)
                    </option>
                    <option value="COMPANY" className="bg-[#0b0f19]">
                      Organizational Entity (Company)
                    </option>
                    <option value="DEAL" className="bg-[#0b0f19]">
                      Revenue Path (Deal)
                    </option>
                  </select>
                </div>
                <div className="col-span-2 flex items-center gap-3 p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                  <input
                    ...
                    className="w-4 h-4 rounded bg-white/5 border-white/10 text-blue-500 focus:ring-0 cursor-pointer"
                  />
                  <label
                    htmlFor="isRequired"
                    className="text-[10px] font-black text-gray-400 uppercase tracking-widest cursor-pointer"
                  >
                    Enforce Data Presence (Required)
                  </label>
                </div>
              </div>
              <div className="pt-6 border-t border-white/5 flex gap-4">
                <button
                  ...
                  className="flex-1 py-4 bg-white/[0.02] border-white/5 border hover:bg-white/[0.05] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all"
                >
                  Abort
                </button>
                <button
                  type="submit"
                  className="flex-1 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-blue-500/20"
                >
                  Execute Injection
                </button>
              </div>
