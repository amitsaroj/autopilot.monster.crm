...
import { Plus, Search, Trash2, Loader2, CheckCircle2, Clock, CheckSquare } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Orchestrate operational follow-ups and engagement cycles
          </p>
        </div>
        <button
        ...
        <div className="space-y-8">
          <div className="space-y-4">
            {tasks.map((task) => (
              <div
                key={task.id}
                className="p-6 rounded-[32px] bg-white/[0.02] border border-white/[0.05] hover:border-indigo-500/20 transition-all group flex items-center gap-6 relative overflow-hidden backdrop-blur-sm"
              >
                <button
            ...
            {tasks.length === 0 && (
              <div className="py-20 text-center">
                <p className="text-gray-500 font-black text-xs uppercase tracking-widest">
                  No operational tasks detected in this cycle.
                </p>
              </div>
            )}
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  >
                    <option value="LOW" className="bg-[#0b0f19]">
                      Minimal Risk (Low)
                    </option>
                    <option value="MEDIUM" className="bg-[#0b0f19]">
                      Standard Ops (Medium)
                    </option>
                    <option value="HIGH" className="bg-[#0b0f19]">
                      Critical Path (High)
                    </option>
                  </select>
                </div>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-indigo-500/40 min-h-[100px] resize-none"
                  placeholder="Provide additional task specifications..."
                />
              </div>
