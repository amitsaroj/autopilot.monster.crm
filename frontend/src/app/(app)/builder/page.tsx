...
export default function BuilderPage() {
  const sections = [
    ...
    },
    {
      ...
      icon: Palette,
      count: 3,
      desc: 'Branding & theme editor',
    },
    {
      label: 'Published',
      href: '/builder/publish',
      icon: Globe,
      count: 6,
      desc: 'Live published pages',
    },
  ];
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {sections.map((s) => (
          <Link
            key={s.label}
            href={s.href}
            className="group rounded-xl border border-border bg-card p-6 hover:border-[hsl(246,80%,60%)]/50 hover:shadow-md transition-all"
          >
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {['Lead Capture Form', 'Pricing Page', 'Contact Us', 'Product Catalog', 'Case Study'].map(
            (page) => (
              <Link
                key={page}
                href="/builder/pages/1/edit"
                className="flex items-center gap-3 p-3 rounded-lg border border-border hover:border-[hsl(246,80%,60%)]/50 transition-colors"
              >
