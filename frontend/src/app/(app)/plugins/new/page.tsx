...
export default function BrowsePluginsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 py-8">
      <Link href="/plugins" className="text-sm text-muted-foreground hover:text-foreground">
        ← Installed Plugins
      </Link>
        ...
        {apps.map((p) => (
          <li key={p.id} className="flex items-center justify-between p-4">
            ...
            <button
              ...
              onClick={() => void handleInstall(p.id)}
              className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-1.5 text-sm text-white disabled:opacity-50"
            >
