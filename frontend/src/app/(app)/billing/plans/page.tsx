  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="text-center">
        ...
        <p className="text-muted-foreground mt-2">Choose a plan that fits your workspace</p>
        <div className="inline-flex items-center gap-2 mt-4 p-1 bg-muted rounded-xl">
          ...
          >
            Annual <span className="text-green-500 font-semibold ml-1">Save</span>
          </button>
        </div>
      </div>
      ...
      {plans.length === 0 ? (
        <div className="rounded-xl border border-border bg-card p-10 text-center">
          <p className="text-sm text-muted-foreground">No active plans are available yet.</p>
          <Link
            href="/billing"
              ...
              >
                ...
                <ul className="space-y-2.5 flex-1 mb-6">
                  <li className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    Monthly: {plan.currency || 'USD'} {Number(plan.priceMonthly).toLocaleString()}
                  </li>
                  <li className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Check className="h-4 w-4 text-green-500 shrink-0" />
                    Annual: {plan.currency || 'USD'} {Number(plan.priceAnnual).toLocaleString()}
                  </li>
                </ul>
                <button
                  type="button"
      ...
      <div className="rounded-xl border border-border bg-card p-6 text-center">
        <h2 className="font-semibold text-foreground mb-1">Manage billing</h2>
        <p className="text-sm text-muted-foreground mb-4">
          View invoices, payment methods, and usage from your billing overview.
        </p>
        <Link
          href="/billing"
