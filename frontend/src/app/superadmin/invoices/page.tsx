...
export default function GlobalInvoicesPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded-full border text-[10px] font-black uppercase tracking-widest ${colors[status]}`}
      >
        {status}
      </span>
    );
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Platform-Wide Revenue Manifest ({invoices.length})
          </p>
        </div>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col items-start gap-1">
                      <span className="text-lg font-black text-white">
                        ...
                        {(inv.total || inv.amount).toLocaleString(undefined, {
                    ...
                    <div className="flex flex-col items-center gap-6 text-gray-700">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-gray-600">
                          Sync with payment provider to populate financial artifacts.
                        </p>
                      </div>
                      <button
                        onClick={fetchInvoices}
                        className="text-indigo-400 text-xs font-black uppercase tracking-widest hover:underline flex items-center gap-2"
                      >
                        Initiate Provider Sync <ArrowRight className="w-4 h-4" />
                      </button>
                    </div>
                  </td>
                </tr>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="p-10 rounded-[40px] bg-white/[0.02] border border-white/[0.05] relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-4">
            ...
            <p className="text-sm text-gray-500 leading-relaxed max-w-sm">
              Generate comprehensive platform-wide tax reports, manage cross-border VAT
              reconciliation, and orchestrate global subscription overrides.
            </p>
            <button className="px-6 py-3 rounded-xl bg-white/[0.03] border border-white/10 text-xs font-black uppercase tracking-widest text-white hover:bg-white/[0.08] transition-all">
              Financial Settings
            </button>
          </div>
        </div>
        <div className="p-10 rounded-[40px] bg-white/[0.02] border border-white/[0.05] relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-4">
            <h3 className="text-2xl font-black text-white text-red-500/80">
              Churn & Deficit Monitoring
            </h3>
            <p className="text-sm text-gray-500 leading-relaxed max-w-sm">
              Review failed payment artifacts, identify high-churn workspace clusters, and provision
              advanced credit-based recovery mechanisms.
            </p>
            <button className="px-6 py-3 rounded-xl bg-red-500/10 border border-red-500/20 text-xs font-black uppercase tracking-widest text-red-400 hover:bg-red-500/20 transition-all">
              Deficit Artifacts
            </button>
          </div>
        </div>
      ...
      <div className="p-8 rounded-3xl border border-dashed border-white/10 bg-white/[0.01] flex items-center gap-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">
            Financial artifacts are cryptographically locked after reconciliation and cannot be
            modified. All updates must be orchestrated via Stripe Provider Hooks to maintain ledger
            integrity.
          </p>
        </div>
      </div>
