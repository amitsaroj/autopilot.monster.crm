...
export default function PlatformApiPerformancePage() {
  ...
  const avgLatency =
    logs.length > 0
      ? (logs.reduce((acc, curr) => acc + curr.durationMs, 0) / logs.length).toFixed(2)
      : 0;
  const errorRate =
    logs.length > 0
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <h1 className="text-3xl font-black text-white tracking-tight">
            API Performance Orchestration
          </h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Monitor Endpoint Latency, Throughput Forensics & Status-Code Persistence
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 p-10 rounded-[60px] bg-white/[0.01] border border-white/[0.05] relative overflow-hidden shadow-2xl">
          ...
          <div className="flex justify-between items-center mb-10 relative">
            <div>
              <h3 className="text-2xl font-black text-white uppercase tracking-tighter leading-none text-sans">
                Endpoint Dispatches
              </h3>
              <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest mt-2">
                Real-Time performance Forensics
              </p>
            </div>
            ...
            {logs.map((log) => (
              ...
              >
                <div className="flex items-center gap-8 flex-1">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xs ${getStatusColor(log.statusCode)}`}
                  >
                    {log.statusCode}
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                      <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest border-b border-indigo-500/20 pb-0.5">
                        {log.method}
                      </span>
                ...
                <div className="flex items-center gap-6 shrink-0 text-right">
                  <div className="space-y-1">
                    <p className="text-[10px] text-gray-700 font-black uppercase tracking-widest leading-none">
                      Latency
                    </p>
                    <p
                      className={`text-lg font-black tracking-tighter ${log.durationMs > 200 ? 'text-amber-500' : 'text-emerald-500'}`}
                    >
                      {log.durationMs}ms
                    </p>
                  </div>
              ...
              {[
                ...
                <div key={ep.label} className="space-y-3">
                  ...
                  <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                    <div
                      className={`h-full ${ep.stress > 80 ? 'bg-red-500' : 'bg-indigo-500'} rounded-full transition-all duration-1000`}
                      style={{ width: `${ep.stress}%` }}
                    />
                  </div>
                </div>
              ))}
          ...
          <div className="p-10 rounded-[60px] bg-white/[0.02] border border-white/[0.05] relative overflow-hidden group shadow-2xl flex flex-col justify-between h-[340px]">
            ...
            <div className="space-y-6 relative">
              ...
              <p className="text-sm text-gray-500 font-medium leading-relaxed uppercase tracking-tight opacity-80 pt-2">
                Platform-wide latency threshold is set to{' '}
                <span className="text-white font-black">200ms</span>. Systemic rebalancing
                dispatches initialize automatically upon policy-divergence.
              </p>
            </div>
