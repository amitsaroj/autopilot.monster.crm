  ...
  const [globalNotifs, setGlobalNotifs] = useState<GlobalNotification[]>([
    {
      id: 'GN-1001',
      scope: 'MAINTENANCE',
      title: 'Global Engine Re-provisioning',
      message:
        'Cluster Alpha nodes will undergo structural maintenance at 04:00 UTC. System persistence will remain active.',
    ...
    },
    {
      id: 'GN-1002',
      scope: 'SECURITY',
      title: 'Identity Shield Upgrade',
      message:
        'Mandatory MFA artifacts will be dispatched to all workspace administrators to ensure lattice integrity.',
    ...
    },
    {
      id: 'GN-1003',
      scope: 'SYSTEM',
      title: 'Marketplace Lattice Expansion',
      message: 'New extension discovery nodes are now active across the global growth cluster.',
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Global-Notification-Hub
            </span>
          </div>
          <h1 className="text-4xl font-black text-white tracking-tight">Platform Communications</h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Orchestrate Structural broadcasts, systemic Alerts & global Engagement forensics
          </p>
        </div>
      ...
      <div className="space-y-6">
        {globalNotifs.map((notif) => (
          <div
            key={notif.id}
            className="p-12 rounded-[60px] bg-white/[0.01] border border-white/[0.05] hover:border-blue-500/20 transition-all group flex flex-col md:flex-row items-center gap-12 relative overflow-hidden shadow-inner"
          >
            ...
            <div
              className={`w-24 h-24 rounded-[40px] bg-white/[0.03] border border-white/5 flex items-center justify-center transition-all shrink-0 shadow-2xl ${notif.scope === 'SECURITY' ? 'text-red-500' : 'text-blue-400'} group-hover:bg-blue-500 group-hover:text-white`}
            >
              {notif.scope === 'SECURITY' ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-5 mb-4 flex-wrap">
                <span
                  className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border transition-colors ${notif.status === 'DISPATCHED' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}
                >
                  {notif.status}
                </span>
              ...
              <p className="text-lg text-gray-500 font-medium leading-relaxed max-w-3xl uppercase tracking-tight opacity-80 pt-2">
                {notif.message}
              </p>
              <div className="flex items-center gap-8 mt-8 text-[11px] text-gray-600 font-black uppercase tracking-widest">
                <span className="flex items-center gap-2 px-1">
                  <Users className="w-4 h-4 opacity-40 shrink-0" /> Recipients: {notif.recipients}
                </span>
                <span className="opacity-20">•</span>
                <span className="flex items-center gap-2">
                  <Clock className="w-4 h-4 opacity-40 shrink-0" /> Dispatch:{' '}
                  {new Date(notif.timestamp).toLocaleTimeString()}
                </span>
                <span className="opacity-20">•</span>
                <span className="flex items-center gap-2">
                  <Globe className="w-4 h-4 opacity-40 shrink-0" /> Scope: {notif.scope} artifact
                </span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mt-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80 pt-2">
              Maintain global platform awareness. Orchestrate structural dispatches across
              geographical node distribution in real-time.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80 pt-2">
              Verify systemic engagement forensics. Track structural broadcast-velocity and
              identity-attribution pulse globally.
            </p>
          </div>
        </div>
      </div>
