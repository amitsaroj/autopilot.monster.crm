...
export default function SystemAuditsPage() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      const res = await api.get(
        `/admin/audit-logs?search=${search}&category=${category}&outcome=${outcome}`,
      );
      const json = res.data;
  ...
  const stats = [
    ...
    {
      label: 'Successful',
      value: logs.filter((l) => l.outcome === 'SUCCESS').length,
      icon: CheckCircle2,
    ...
    {
      label: 'Failures',
      value: logs.filter((l) => l.outcome === 'FAILURE').length,
      icon: XCircle,
      color: 'text-red-400',
      bg: 'bg-red-500/10',
    },
    {
      label: 'Warnings',
      value: logs.filter((l) => l.outcome === 'WARNING').length,
      icon: AlertTriangle,
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Platform-Wide Security Event Chronicle
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          <div
            key={s.label}
            className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] flex items-center gap-4 group hover:bg-white/[0.04] transition-all"
          >
                    ...
                    <tr key={log.id} className="group hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <div>
                          ...
                          {log.resourceId && (
                            <p className="text-[10px] font-mono text-gray-600 mt-0.5 truncate max-w-[120px]">
                              {log.resourceId}
                            </p>
                          )}
                        </div>
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`px-2.5 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 w-fit ${OUTCOME_STYLES[log.outcome]}`}
                        >
