...
const DEMO_FEATURES = [
  {
    icon: Users,
    label: 'CRM & Contacts',
    desc: 'Full contact, company and deal management',
    color: 'text-blue-400',
    bg: 'bg-blue-500/10',
  },
  {
    icon: Brain,
    label: 'AI Agents',
    desc: 'Autonomous AI sales and support agents',
  ...
  },
  {
    ...
    desc: 'Visual automation builder with 100+ triggers',
  ...
  },
  {
    ...
    desc: 'Unified omnichannel messaging center',
  ...
  },
  {
    icon: Phone,
    label: 'Voice Cloud',
    desc: 'Outbound dialer, IVR, and call analytics',
    color: 'text-cyan-400',
    bg: 'bg-cyan-500/10',
  },
  {
    ...
    desc: 'Revenue dashboards and pipeline insights',
  ...
  },
  {
    ...
    desc: 'Subscriptions, invoices, and usage billing',
  ...
  },
  {
    icon: Globe,
    label: 'Marketplace',
    desc: 'Install plugins and third-party integrations',
    color: 'text-rose-400',
    bg: 'bg-rose-500/10',
  },
];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Provision Live Demo Environments for Prospects
          </p>
        </div>
        <button
        ...
        <div className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Demo tenant provisioned with seed data. Access at{' '}
              <span className="text-indigo-400 font-mono">demo.autopilotmonster.com</span>
            </p>
          </div>
          <a
            href="https://demo.autopilotmonster.com"
            target="_blank"
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {DEMO_FEATURES.map((f) => (
            <div
              key={f.label}
              className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] hover:border-indigo-500/20 transition-all group cursor-pointer"
            >
              <div
                className={`p-3 rounded-xl ${f.bg} w-fit mb-4 group-hover:scale-110 transition-transform`}
              >
            ...
            <tbody className="divide-y divide-white/[0.03]">
              {DEMO_TENANTS.map((t) => (
                <tr key={t.name} className="group hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`px-2.5 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest ${t.status === 'ACTIVE' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20'}`}
                    >
                      {t.status}
                    </span>
                  </td>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-500/10 via-transparent to-transparent border border-indigo-500/20">
          ...
          <div className="space-y-3">
            {[
              ...
              <button
                key={action.label}
                className="w-full text-left p-3 rounded-xl bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.08] hover:border-indigo-500/20 transition-all group"
              >
