    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Catalog-Orchestrator
            </span>
          </div>
          <h1 className="text-4xl font-black text-white tracking-tight">Marketplace Catalog</h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Orchestrate Taxonomy Artifacts, Featured extension dispatches & structural persistence
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        {[
          ...
          {
            label: 'Extensions Tracked',
            value: categories.reduce((acc, c) => acc + c.count, 0),
            icon: Package,
        ...
        <div className="lg:col-span-2 p-12 rounded-[60px] bg-white/[0.01] border border-white/[0.05] relative overflow-hidden group shadow-inner">
          ...
          <div className="flex justify-between items-start mb-10">
            <div>
              ...
              <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest mt-2 px-1">
                Forensics: Structural Categories
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
            {categories.map((cat) => (
              <div
                key={cat.id}
                className="p-8 rounded-[40px] bg-white/[0.02] border border-white/5 hover:border-indigo-500/20 transition-all group/item flex flex-col justify-between h-[220px]"
              >
                <div className="flex justify-between items-start">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-[9px] font-black text-gray-700 uppercase tracking-widest group-hover/item:text-white/40 transition-colors">
                    {cat.status}
                  </span>
                </div>
                <div>
                  <h4 className="text-xl font-black text-white uppercase tracking-tight group-hover/item:text-indigo-400 transition-colors truncate">
                    {cat.name}
                  </h4>
                  <div className="flex items-center justify-between mt-4">
                    <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest leading-none shrink-0">
                      {cat.count} Artifacts Tracked
                    </p>
        ...
        <div className="p-10 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col justify-between relative overflow-hidden shadow-2xl h-full">
          ...
          <div className="space-y-10 relative">
            ...
            <div className="space-y-6">
              {collections.map((col) => (
                <div
                  key={col.id}
                  className="p-8 rounded-[40px] bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] transition-all group/row flex flex-col justify-between h-[180px]"
                >
                  <div className="flex justify-between items-start">
                    <h4 className="text-lg font-black text-white uppercase tracking-tight leading-none group-hover/row:text-purple-400 transition-colors">
                      {col.name}
                    </h4>
                  ...
                  <div className="flex justify-between items-center text-[10px] text-gray-600 font-black uppercase tracking-widest italic">
                    ...
                    <button className="flex items-center gap-2 text-indigo-400 hover:underline">
                      Manage Dispatch <MoveRight className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="p-10 rounded-[45px] bg-indigo-500 text-white space-y-4 shadow-2xl shadow-indigo-500/20 relative overflow-hidden cursor-pointer group/promo mt-10">
            ...
            <div className="flex items-center gap-6 mb-2">
              ...
              <h4 className="text-2xl font-black uppercase tracking-tighter leading-none text-sans">
                Promotion Lattices
              </h4>
            </div>
            <p className="text-xs text-white/80 leading-relaxed font-bold uppercase tracking-tight">
              Orchestrate global marketplace discovery dispatches. Deploy featured extension
              artifacts across all platform discovery nodes.
            </p>
          </div>
        </div>
      </div>
