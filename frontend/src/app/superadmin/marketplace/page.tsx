    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {plugins.map((plugin) => (
          <div
            key={plugin.id}
            className="p-8 rounded-[40px] bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] hover:border-indigo-500/20 transition-all group relative overflow-hidden flex flex-col justify-between"
          >
            ...
            <div>
              ...
              <div className="space-y-1 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded-md bg-white/[0.05] border border-white/5 text-[8px] font-black text-gray-500">
                    v{plugin.version}
                  </span>
                </div>
                <p className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase whitespace-nowrap overflow-hidden text-ellipsis max-w-[200px]">
                  ID: {plugin.id.substring(0, 16)}...
                </p>
              </div>

              <p className="text-sm text-gray-500 leading-relaxed line-clamp-3 mb-8">
                {plugin.description ||
                  'Advanced platform module provisioned with systemic permissions and cross-tenant execution capabilities.'}
              </p>
            </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-2xl bg-black/40 border border-white/5 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[9px] text-gray-600 font-black uppercase tracking-tighter leading-tight">
                      Installs
                    </p>
              ...
              <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest px-1">
                ...
                <span
                  className={`flex items-center gap-1.5 ${plugin.active ? 'text-emerald-500' : 'text-gray-500'}`}
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${plugin.active ? 'bg-emerald-500 shadow-lg shadow-emerald-500/50' : 'bg-gray-500'} `}
                  />
          ...
          <div className="flex items-center gap-6 mb-6">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest">
                Master-Node Synchronized
              </p>
            </div>
          </div>
          <p className="text-sm text-gray-500 leading-relaxed max-w-sm font-medium">
            Orchestrate plugin availability across regional clusters, manage global versioning
            artifacts and verify cross-platform security seals.
          </p>
        </div>
        <div className="p-10 rounded-[40px] bg-white/[0.01] border border-white/[0.05] relative overflow-hidden group">
          ...
          <p className="text-sm text-gray-500 leading-relaxed max-w-sm font-medium">
            All marketplace artifacts undergo systemic sandbox verification and static code analysis
            before admission to the global catalog.
          </p>
        </div>
      </div>
    </div>
