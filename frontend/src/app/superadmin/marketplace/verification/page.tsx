...
export default function MarketplaceReviewQueuePage() {
  const [submissions, setSubmissions] = useState<Submission[]>([
    ...
    {
      id: '2',
      name: 'CRM Forensic Toolkit',
      version: '1.0.2',
      developer: 'DeepMind Ext',
      category: 'Analytics',
      submittedAt: new Date(Date.now() - 86400000).toISOString(),
      status: 'UNDER_CORE_AUDIT',
      riskLevel: 'MEDIUM',
    },
    {
      id: '3',
      name: 'Global Outreach Node',
      version: '0.9.8',
      developer: 'Unknown Actor',
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Verification-Orchestrator
            </span>
          </div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Orchestrate Structural Audits, developer Identity-Attribution & ecosystem Integrity
            dispatches
          </p>
        </div>
      ...
      <div className="space-y-6">
        {submissions.map((sub) => (
          <div
            key={sub.id}
            className="p-10 rounded-[60px] bg-white/[0.01] border border-white/[0.05] hover:border-indigo-500/20 transition-all group flex flex-col lg:flex-row items-center gap-10 relative overflow-hidden"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-4 mb-3 flex-wrap">
                <span
                  className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border ${sub.status === 'UNDER_CORE_AUDIT' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}
                >
                  {sub.status.replace('_', ' ')}
                </span>
                <span
                  className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border ${sub.riskLevel === 'CRITICAL' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-white/5 text-gray-500 border-white/10'}`}
                >
                  RISK: {sub.riskLevel}
                </span>
              ...
              <div className="flex items-center gap-6 text-[11px] text-gray-600 font-black uppercase tracking-widest leading-none">
                <span className="flex items-center gap-2">
                  <User className="w-4 h-4 opacity-40 shrink-0" /> {sub.developer}
                </span>
                <span className="opacity-20 hidden md:block">•</span>
                <span className="flex items-center gap-2 hidden md:flex">
                  <Layers className="w-4 h-4 opacity-40 shrink-0" /> {sub.category} artifact
                </span>
                <span className="opacity-20 hidden md:block">•</span>
                <span className="flex items-center gap-2">
                  <History className="w-4 h-4 opacity-40 shrink-0" /> v{sub.version}
                </span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Analyze structural extension dispatches for vulnerabilities. Every certificated
              artifact must pass platform integrity verification.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Execute pressure audit dispatches on extension artifacts. Verify performance
              persistence across theoretical global resource clusters.
            </p>
          </div>
        </div>
      </div>
