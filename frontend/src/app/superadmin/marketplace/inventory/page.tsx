...
function mapPlugin(plugin: AdminPlugin): Plugin {
  const status: Plugin['status'] = plugin.status === 'ACTIVE' ? 'PUBLISHED' : 'DRAFT';
  return {
    id: plugin.id,
    name: plugin.name,
...
export default function MarketplaceInventoryPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${caps[status]}`}
      >
        {status.replace('_', ' ')}
      </span>
    );
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Extension-Orchestrator
            </span>
          </div>
          <h1 className="text-3xl font-black text-white tracking-tight">Marketplace Inventory</h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Orchestrate Global Extension artifacts, Installation Forensics & Developer Persistence
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          {
            label: 'Platform Installs',
            value: plugins.reduce((acc, p) => acc + (p.installCount || 0), 0),
            icon: Download,
            color: 'text-emerald-500',
            bg: 'bg-emerald-500/10',
          },
          {
            label: 'Developer Lattices',
            value: '12 Active',
            icon: Code,
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="p-4 bg-white/[0.02] border border-white/5 rounded-xl text-gray-500 hover:text-white transition-all shadow-lg"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {plugins.map((plugin) => (
          <div
            key={plugin.id}
            className="p-8 rounded-[40px] bg-white/[0.01] border border-white/[0.05] hover:border-indigo-500/20 transition-all group relative overflow-hidden flex flex-col justify-between h-[450px]"
          >
            ...
            <div>
              ...
              <div className="space-y-4">
                <h3 className="text-2xl font-black text-white group-hover:text-indigo-400 transition-colors uppercase tracking-tight mb-2 leading-none">
                  {plugin.name || 'Untitled Extension'}
                </h3>
                <p className="text-sm text-gray-600 font-medium leading-relaxed line-clamp-2 uppercase tracking-wide opacity-80">
                  {plugin.description ||
                    'No structural description provided for this extension artifact.'}
                </p>
            ...
            <div className="space-y-6 pt-6 border-t border-white/5">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2 text-[10px] text-gray-700 font-black uppercase tracking-widest shrink-0">
                  <Download className="w-4 h-4 opacity-40 shrink-0" /> {plugin.installCount || 0}{' '}
                  Installs
                </div>
                <div className="flex items-center gap-2 text-[10px] text-gray-700 font-black uppercase tracking-widest shrink-0">
              ...
              <div className="flex gap-4">
                <button className="flex-1 py-5 rounded-2xl bg-white/[0.03] border border-white/5 text-[10px] font-black text-white uppercase tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2 group/btn">
                  <Edit2 className="w-4 h-4 text-indigo-400 group-hover/btn:scale-110 transition-transform" />{' '}
                  Manage Node
                </button>
        ...
        <div className="p-10 rounded-[60px] border border-dashed border-white/10 bg-indigo-500/[0.01] flex flex-col items-center justify-center text-center space-y-8 group cursor-pointer hover:border-indigo-500/30 transition-all h-[450px]">
          ...
          <div>
            <h3 className="text-2xl font-black text-white uppercase tracking-tighter leading-tight font-sans">
              Initialize Extension
            </h3>
            <p className="text-sm text-gray-600 max-w-[240px] mx-auto leading-relaxed mt-3 uppercase tracking-widest font-black opacity-30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mt-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Automated structural security dispatches verify every extension artifact. Ensure
              absolute integrity across your platform extension lattice.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Analyze installation velocity and revenue persistence for every extension artifact.
              Orchestrate high-fidelity marketplace growth dispatches.
            </p>
          </div>
        </div>
      </div>
