...
export default function TenantManagementPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${colors[status] ?? colors.TRIAL}`}
      >
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Managed Clusters ({tenants.length})
          </p>
        </div>
      ...
      <div className="rounded-3xl border border-white/[0.05] bg-white/[0.01] overflow-hidden shadow-2xl">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/[0.05] bg-white/[0.02]">
                ...
                <th className="px-6 py-5 text-[10px] font-black text-gray-500 uppercase tracking-widest text-right">
                  Actions
                </th>
              </tr>
            </thead>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-600 font-mono mt-0.5">
                          {tenant.slug}.autopilotmonster.com
                        </p>
                      </div>
                    </div>
                  </td>
        ...
        <div className="px-6 py-4 bg-white/[0.02] border-t border-white/[0.05] flex items-center justify-between text-xs text-gray-600">
          <span>
            Showing {tenants.length} of {tenants.length} Entities
          </span>
          <div className="flex items-center gap-4">
            <button
              disabled
              className="hover:text-white disabled:opacity-30 flex items-center gap-1 transition-colors"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="p-8 rounded-3xl bg-white/[0.02] border border-white/[0.05] hover:border-indigo-500/20 transition-all group">
          ...
          <p className="text-sm text-gray-500 leading-relaxed mb-6">
            Set cluster-wide resource ceilings for storage, task execution minutes, and AI token
            consumption across all active tenants.
          </p>
          <Link
            href="/superadmin/plans"
            className="text-indigo-400 text-xs font-black flex items-center gap-2 uppercase tracking-widest group-hover:translate-x-1 transition-transform"
          >
        ...
        <div className="p-8 rounded-3xl bg-white/[0.02] border border-white/[0.05] hover:border-amber-500/20 transition-all group">
          ...
          <p className="text-sm text-gray-500 leading-relaxed mb-6">
            Modify plan DTOs, update price per unit mappings, and provision promotional custom plans
            for enterprise partners.
          </p>
          <Link
            href="/superadmin/plans"
            className="text-amber-400 text-xs font-black flex items-center gap-2 uppercase tracking-widest group-hover:translate-x-1 transition-transform"
          >
