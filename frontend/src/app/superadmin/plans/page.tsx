    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Billing Architecture ({plans.length} tiers)
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {plans.map((plan) => (
          <div
            key={plan.id}
            className="group p-8 rounded-3xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] hover:border-indigo-500/20 transition-all relative overflow-hidden flex flex-col justify-between"
          >
            ...
            <div>
              <div className="flex justify-between items-start mb-6">
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${plan.status === 'ACTIVE' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-gray-500/10 text-gray-500 border-gray-500/20'}`}
                    >
                      {plan.status}
                    </span>
                  </div>
              ...
              <p className="text-sm text-gray-400 leading-relaxed mb-8 max-w-md">
                {plan.description ||
                  'No detailed tier description provided. Enterprise settings active.'}
              </p>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span
                    className={`font-bold ${plan.isPublic ? 'text-indigo-400' : 'text-amber-400'}`}
                  >
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="text-gray-500 flex items-center gap-2">
                    <CreditCard className="w-3.5 h-3.5" /> Stripe Sync
                  </span>
        ...
        <button
          onClick={() => setShowCreateModal(true)}
          className="group p-8 rounded-3xl border border-dashed border-white/10 hover:border-indigo-500/40 hover:bg-indigo-500/[0.02] transition-all flex flex-col items-center justify-center text-center space-y-4"
        >
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 px-10">
              Provision a custom enterprise-grade tier with specific resource quotas and
      ...
      <div className="p-8 rounded-3xl bg-amber-500/5 border border-amber-500/20 flex items-start gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            Changes to these pricing tiers must be manually synchronized with your Stripe Dashboard.
            Deleting a plan will NOT cancel existing subscriptions but will prevent new
            registrations for that tier. Always archive plans before deletion to maintain audit
            integrity.
          </p>
        </div>
      </div>
          ...
          <div className="bg-[#111] border border-white/10 p-8 rounded-2xl w-full max-w-md space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewPlan({ ...newPlan, priceMonthly: Number(e.target.value) })
                    }
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={newPlan.priceAnnual}
                    onChange={(e) =>
                      setNewPlan({ ...newPlan, priceAnnual: Number(e.target.value) })
                    }
                    className="w-full bg-black border border-white/10 rounded-lg px-4 py-2 text-white"
                  />
                </div>
            ...
            <div className="flex gap-4 justify-end pt-4 border-t border-white/10">
              ...
              <button
                ...
                className="px-4 py-2 bg-indigo-500 hover:bg-indigo-400 text-white text-sm font-bold rounded-lg transition-colors"
              >
                Create Plan
              </button>
            </div>
