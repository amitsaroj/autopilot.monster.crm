...
export default function PlatformEventBusForensicsPage() {
  const [events, setEvents] = useState<PlatformEvent[]>([
    ...
    },
    {
      ...
      source: 'Monetization-Hub',
      timestamp: new Date(Date.now() - 60000).toISOString(),
      status: 'DISPATCHED',
    },
    {
      id: 'EV-8803',
      name: 'WORKFLOW_DIVERGENCE',
      source: 'Flow-Engine',
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Event-Bus-Forensics
            </span>
          </div>
          <h1 className="text-4xl font-black text-white tracking-tight">Event Bus Forensics</h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Monitor structural Dispatches, systemic Pub/Sub forensics & event Persistence
          </p>
        </div>
      ...
      <div className="p-10 rounded-[60px] bg-white/[0.01] border border-white/[0.05] relative overflow-hidden group shadow-inner">
        ...
        <div className="flex justify-between items-center mb-10 relative px-2">
          <div>
            <h3 className="text-2xl font-black text-white uppercase tracking-tighter text-sans leading-none">
              Systemic Dispatches
            </h3>
            <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest mt-2">
              Forensics: Event Persistence
            </p>
          </div>
        ...
        <div className="space-y-4 relative max-h-[600px] overflow-y-auto custom-scrollbar pr-4">
          {events.map((ev) => (
            <div
              key={ev.id}
              className="p-6 rounded-[32px] bg-white/[0.02] border border-white/[0.05] hover:border-blue-500/20 transition-all group flex flex-col md:flex-row items-center gap-8 relative overflow-hidden"
            >
              ...
              <div className="flex items-center gap-6 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                    <span className="px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-[9px] font-black text-gray-700 uppercase tracking-widest">
                      {ev.id}
                    </span>
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${ev.status === 'PROCESSED' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}
                    >
                      {ev.status}
                    </span>
                  </div>
                  <p className="text-[10px] text-gray-600 font-medium uppercase tracking-widest italic flex items-center gap-2">
                    <Terminal className="w-3.5 h-3.5 opacity-40 shrink-0" /> Source: {ev.source}{' '}
                    Node
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mt-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Analyze structural pub/sub dispatches. Track systemic event-velocity and topology
              persistence across global resource clusters.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-10 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Orchestrate high-fidelity event dispatches. Maintain structural connectivity across
              your platform's geographical node distribution.
            </p>
          </div>
        </div>
      </div>
