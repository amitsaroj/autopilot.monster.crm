    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Storage-Persistence-Orchestrator
            </span>
          </div>
          <h1 className="text-4xl font-black text-white tracking-tight">Storage Persistence</h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Orchestrate Structural data dispatches, bucket forensics & systemic artifact persistence
          </p>
        </div>
        ...
        {[
          {
            ...
            value: nodes
              .reduce(
                (acc, n) =>
                  acc + (n.count.includes('k') ? parseFloat(n.count) * 1000 : parseFloat(n.count)),
                0,
              )
              .toLocaleString(),
            icon: Files,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        {nodes.map((node) => (
          <div
            key={node.id}
            className="p-12 rounded-[60px] bg-white/[0.01] border border-white/[0.05] hover:border-blue-500/20 transition-all group flex flex-col justify-between h-[480px] relative overflow-hidden shadow-inner"
          >
            ...
            <div>
              <div className="flex justify-between items-start mb-10">
                ...
                <div className="flex gap-4">
                  <span
                    className={`px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest ${node.status === 'ONLINE' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}
                  >
                    {node.status}
                  </span>
              ...
              <div className="space-y-4">
                <h3 className="text-3xl font-black text-white uppercase tracking-tight leading-none group-hover:text-blue-400 transition-colors">
                  {node.name}
                </h3>
                <p className="text-[12px] text-gray-600 font-mono italic uppercase tracking-tighter flex items-center gap-2">
                  <Globe className="w-4 h-4 opacity-40 shrink-0" /> {node.provider} Node •{' '}
                  {node.region}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-10 pt-10 border-t border-white/5 relative">
              <div className="grid grid-cols-2 gap-10">
                <div className="space-y-2">
                  <p className="text-[11px] text-gray-700 font-black uppercase tracking-widest mb-1.5 opacity-60">
                    Artifact count
                  </p>
                ...
                <div className="space-y-2 text-right">
                  ...
                  <p className="text-2xl font-black text-blue-400 uppercase tracking-tighter italic">
                    {node.size}
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mt-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80 pt-2">
              Analyze structural data dispatches. Monitor artifact persistence and systemic
              throughput across global storage nodes in real-time.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80 pt-2">
              Maintain global data persistence. Synchronize structural integrity artifacts across
              theoretical geographical storage clusters.
            </p>
          </div>
        </div>
      </div>
