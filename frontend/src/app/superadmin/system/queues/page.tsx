    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        {[
          {
            ...
            value: queues.reduce((acc, q) => acc + q.count, 0),
            icon: Layers,
          ...
          },
          {
            ...
            value: queues.reduce((acc, q) => acc + q.activeCount, 0),
            icon: Activity,
          ...
          },
          {
            ...
            value: queues.reduce((acc, q) => acc + q.failedCount, 0),
            icon: AlertTriangle,
            color: 'text-red-500',
            bg: 'bg-red-500/10',
          },
          {
            label: 'Engine Persistence',
            value: '99.8%',
            icon: ShieldCheck,
            color: 'text-blue-500',
            bg: 'bg-blue-500/10',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        {queues.map((queue) => (
          <div
            key={queue.id}
            className="p-10 rounded-[60px] bg-white/[0.01] border border-white/[0.05] hover:border-amber-500/20 transition-all group flex flex-col justify-between h-[420px] relative overflow-hidden"
          >
            ...
            <div>
              <div className="flex justify-between items-start mb-8">
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${queue.paused ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}
                  >
              ...
              <h3 className="text-2xl font-black text-white group-hover:text-amber-400 transition-colors uppercase tracking-tight mb-2 leading-none">
                {queue.name}
              </h3>
              <p className="text-[11px] text-gray-600 font-black uppercase tracking-widest opacity-80 flex items-center gap-2 mb-8">
                <Terminal className="w-3.5 h-3.5 opacity-40 shrink-0" /> {queue.type} Node • Latency
                Isolation Active
              </p>
              ...
              <div className="grid grid-cols-3 gap-6 pt-6 border-t border-white/5">
                ...
                <div>
                  ...
                  <p className="text-xl font-black text-red-500 uppercase tracking-tighter">
                    {queue.failedCount}
                  </p>
                </div>
                <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mt-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Execute structural job dispatches across the platform. Synchronize high-velocity
              processing artifacts across global resource clusters.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            <h3 className="text-3xl font-black text-white uppercase tracking-tighter font-sans leading-none">
              Dead-Letter Forensics
            </h3>
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80">
              Analyze failure artifacts in real-time. Identify structural job divergences and
              initialize recovery dispatches automatically across the lattice.
            </p>
          </div>
        </div>
      </div>
