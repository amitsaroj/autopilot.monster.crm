    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 text-sans">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] text-gray-600 font-mono tracking-tighter uppercase shrink-0 overflow-hidden text-ellipsis max-w-[200px]">
              Node: Worker-Cluster-Orchestrator
            </span>
          </div>
          <h1 className="text-4xl font-black text-white tracking-tight">
            Worker Cluster Orchestration
          </h1>
          <p className="text-gray-500 text-sm mt-1 uppercase tracking-widest font-bold">
            Monitor Structural consumption, node behavior Forensics & systemic execution dispatches
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        {workers.map((worker) => (
          <div
            key={worker.id}
            className="p-12 rounded-[60px] bg-white/[0.01] border border-white/[0.05] hover:border-emerald-500/20 transition-all group flex flex-col justify-between h-[500px] relative overflow-hidden shadow-inner"
          >
            ...
            <div>
              <div className="flex justify-between items-start mb-10">
                ...
                <div className="flex gap-4">
                  <span
                    className={`px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest ${worker.status === 'ACTIVE' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-white/5 text-gray-500 border-white/10'}`}
                  >
                    {worker.status}
                  </span>
              ...
              <div className="space-y-4">
                <h3 className="text-3xl font-black text-white uppercase tracking-tight leading-none group-hover:text-emerald-400 transition-colors">
                  {worker.name}
                </h3>
                <p className="text-[12px] text-gray-600 font-mono italic uppercase tracking-tighter flex items-center gap-2">
                  <Globe className="w-4 h-4 opacity-40 shrink-0" /> {worker.hostname} • {worker.id}{' '}
                  Node
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-10">
                <div className="space-y-4">
                  <div className="flex justify-between text-[11px] text-gray-700 font-black uppercase tracking-widest px-1">
                    ...
                    <span className={worker.cpuUsage > 80 ? 'text-red-500' : 'text-emerald-400'}>
                      {worker.cpuUsage}%
                    </span>
                  </div>
                  <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden shadow-inner">
                    <div
                      className={`h-full ${worker.cpuUsage > 80 ? 'bg-red-500' : 'bg-emerald-500'} rounded-full transition-all duration-1000`}
                      style={{ width: `${worker.cpuUsage}%` }}
                    />
                  </div>
                </div>
                <div className="space-y-4 text-right">
                  <p className="text-[11px] text-gray-700 font-black uppercase tracking-widest mb-1.5 opacity-60 leading-none">
                    Task Persistence
                  </p>
        ...
        <div className="p-12 rounded-[60px] border border-dashed border-white/10 bg-emerald-500/[0.01] flex flex-col items-center justify-center text-center space-y-10 group cursor-pointer hover:border-emerald-500/30 transition-all h-[500px]">
          ...
          <div>
            <h3 className="text-3xl font-black text-white uppercase tracking-tighter leading-tight font-sans">
              Initialize Executor
            </h3>
            <p className="text-sm text-gray-600 max-w-[280px] mx-auto leading-relaxed mt-4 uppercase tracking-widest font-black opacity-30 italic leading-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mt-10">
        <div className="p-12 rounded-[60px] bg-white/[0.02] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80 pt-2">
              Analyze structural executor dispatches. Monitor binary consumption dispatches across
              global resource nodes in real-time.
            </p>
          </div>
        </div>
        <div className="p-12 rounded-[60px] bg-[#0b0f19] border border-white/[0.05] flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden shadow-2xl">
          ...
          <div className="space-y-4 relative">
            ...
            <p className="text-base text-gray-500 font-medium leading-relaxed max-w-sm uppercase tracking-tight opacity-80 pt-2">
              Maintain global mechanical persistence. Synchronize high-velocity processing artifacts
              across theoretical hardware clusters.
            </p>
          </div>
        </div>
      </div>
