        ...
        <div className="relative z-10 space-y-10">
          ...
          <div>
            <h1 className="text-4xl font-black text-white tracking-tighter uppercase mb-2 leading-none">
              Select Growth Vector
            </h1>
            <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest">
              Onboarding Phase 03: Fiscal Layer Provisioning
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans.map((plan) => (
              ...
              >
                {plan.recommended && (
                  <div className="absolute top-6 right-6 px-3 py-1 bg-violet-500 text-white text-[8px] font-black uppercase tracking-widest rounded-full shadow-lg">
                    Recommended
                  </div>
                )}
                ...
                <div
                  className={cn(
                    'w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center mb-10 transition-all',
                    selectedPlan === plan.id ? 'bg-violet-500 text-white' : plan.color,
                  )}
                >
                  <plan.icon className="w-6 h-6" />
                </div>

                <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2 group-hover:text-violet-400 transition-colors">
                  {plan.name}
                </h3>
                ...
                <ul className="space-y-4 mb-10">
                  {plan.features.map((f, i) => (
                    <li
                      key={i}
                      className="flex items-center gap-3 text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none"
                    >
                      <Check
                        className={cn(
                          'w-4 h-4',
                          selectedPlan === plan.id ? 'text-violet-400' : 'text-gray-600',
                        )}
                      />{' '}
                      {f}
                    </li>
