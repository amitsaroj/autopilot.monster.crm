    ...
    <div className="space-y-6">
      ...
      {isCreating && (
        <form
          ...
          className="bg-white dark:bg-card p-6 rounded-2xl border-2 border-indigo-100 dark:border-indigo-900/30 shadow-xl shadow-indigo-500/5 space-y-4"
        >
          <input
            required
      ...
      <div className="grid grid-cols-1 gap-4">
        {notes.length === 0 && !isCreating ? (
          ...
          notes.map((note) => (
            <div
              key={note.id}
              className="group bg-white dark:bg-card p-5 rounded-2xl border border-gray-100 dark:border-border hover:border-indigo-100 dark:hover:border-indigo-900/50 shadow-soft transition"
            >
                ...
                {note.tags.length > 0 && (
                  <div className="flex items-center gap-1.5">
                    ...
                    {note.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-[9px] font-black text-indigo-500 uppercase tracking-tighter"
                      >
                        #{tag}
                      </span>
                    ))}
