...
export const FormNode = ({ data, selected }: any) => {
  return (
    <div
      className={`px-4 py-3 rounded-xl border-2 transition-all min-w-[200px] ${
        selected
    ...
    >
      ...
      <div className="flex items-center gap-2 pb-2 mb-2 border-b border-gray-100 dark:border-white/5">
        ...
        <span className="text-xs font-bold dark:text-gray-300 uppercase tracking-wider">
          Form Extraction
        </span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-1">
          {['Name', 'Email', 'Company', 'Budget'].map((f) => (
            <span
              key={f}
              className="px-2 py-0.5 rounded bg-gray-100 dark:bg-white/5 text-[9px] font-bold text-gray-600 dark:text-gray-400 uppercase"
            >
              {f}
            </span>
          ))}
        ...
        <div className="p-2 rounded-lg bg-green-500/5 border border-green-500/10 flex items-center gap-2">
          ...
          <span className="text-[9px] font-bold text-green-700 dark:text-green-500 uppercase tracking-tighter">
            Auto-sync with CRM
          </span>
        </div>
      </div>
